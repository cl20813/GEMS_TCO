...
class VecchiaBatched(SpatioTemporalModel):
    ...
    def vecchia_batched_likelihood(self, params):
        ...
        # 1. Heads (Exact GP) - Uses the optimized function above
        ...
        # 2. Tails (Batched + Mini-Batched Execution)
        # ---------------------------------------------------
        chunk_size = 4000  # Adjust based on GPU memory
        total_points = self.X_batch.shape[0]
        vecchia_nll_sum = 0.0
        
        # Loop prevents OOM on large datasets
        for start_idx in range(0, total_points, chunk_size):
            end_idx = min(start_idx + chunk_size, total_points)
            
            # Slice current chunk
            X_chunk = self.X_batch[start_idx:end_idx]       # (Batch, M, 3)
            Y_chunk = self.Y_batch[start_idx:end_idx]       # (Batch, M, 1)
            Locs_chunk = self.Locs_batch[start_idx:end_idx] # (Batch, M, 3)
            
            # Compute Covariance (Only for this chunk!)
            ...
            # --- YOUR OPTIMIZED LOGIC (Applied to Chunk) ---
            
            # 1. Forward Solve Only (Transform to Z-space)
            ...
            # 2. Compute GLS Components (Z^T @ Z)
        ...
        total_count = self.Heads_data.shape[0] + total_points
