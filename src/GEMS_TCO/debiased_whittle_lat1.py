...
class debiased_whittle_preprocess(full_vecc_dw_likelihoods):
    ...
    def apply_first_difference_2d_tensor(self, df_tensor: torch.Tensor) -> torch.Tensor:
        """
        Applies 1D lat-direction first-difference filter.
        Z(i,j) = X(i+1,j) - X(i,j).  Output grid: (M-1) x N.
        H(w1,w2) = e^{-iw1} - 1 → zero at entire w1=0 row.
        """
        ...
        # Lat-only 1-1 difference: Z(i,j) = -X(i,j) + X(i+1,j)
        ...
        # Lat shrinks by 1; lon stays full
        new_lats = unique_lats[:-1]
        new_lons = unique_lons
...
class debiased_whittle_likelihood: # (full_vecc_dw_likelihoods):
    ...
    def cov_spatial_difference(u1, u2, t, params, delta1, delta2):
        ...
        """
        weights = {(0, 0): -1.0, (1, 0): 1.0}  # lat-only 1-1 difference
        device = params.device
    ...
    def whittle_likelihood_loss_tapered(params, I_sample, n1, n2, p_time, taper_autocorr_grid, delta1, delta2):
        ...
        # Exclude w1=0 row: H(w1,w2)=0 when w1=0 for lat-direction difference
        row0_sum = likelihood_terms[0, :].sum() if n1 > 0 else torch.tensor(0.0, device=device, dtype=torch.float64)
        ...
        num_terms = n1 * n2 - n2  # exclude entire w1=0 row (n2 terms)
        if num_terms > 0:
