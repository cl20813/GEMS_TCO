"""Private corridor-neighbor geometry for the maintained Vecchia models.

This module reuses the block likelihood, GLS mean estimation, covariance,
batching, and optimizer from :class:`~GEMS_TCO.vecchia.grouped_batched.GroupedBatchedVecchia`
and implements only the publication corridor design.  Current-time neighbors
are previous clusters in max-min order.  Lagged neighbors cover either a
fixed-longitude corridor or, in directional subclasses, a signed two-dimensional
corridor derived from a reference advection vector.

The former center, shifted-center, force-center, and non-corridor geometry
experiments are intentionally absent from the installed package. Historical
material belongs in the non-installed research archive.
...
Important geometry convention:
  ...
  previous-in-order clusters. This distinction is required by temporal
  precedence and is enforced directly by the corridor implementation.
"""
...
from .._base import _integer_option
from ..grouped_batched import GroupedBatchedVecchia


class _CorridorClusterVecchia(GroupedBatchedVecchia):
    """Cluster Vecchia restricted to the maintained corridor geometry."""
    ...
    def __init__(
        ...
        lag1_block_count: int = 4,
        lag2_block_count: int = 3,
        ...
        self.lag0_block_count = _integer_option("lag0_block_count", lag0_block_count, minimum=1)
        self.lag1_block_count = _integer_option("lag1_block_count", lag1_block_count, minimum=1)
        self.lag2_block_count = _integer_option("lag2_block_count", lag2_block_count, minimum=1)
        self.block_row_offset = _integer_option("block_row_offset", block_row_offset)
        self.block_col_offset = _integer_option("block_col_offset", block_col_offset)

        lag1_offset_arg = float(abs(lag1_lon_offset))
        lag2_offset_arg = (
            float(abs(lag2_lon_offset)) if lag2_lon_offset is not None else 2.0 * lag1_offset_arg
        )
        ...
        self.all_neighbor_search = _integer_option(
    ...
    def _clean_lon_interval(
        ...
        if not np.isfinite(lo) or not np.isfinite(hi):
            raise ValueError("corridor interval endpoints must be finite")
        lo = float(abs(lo))
        hi = float(abs(hi))
    ...
    def _cluster_candidates_from_center(self, center_block: int, count: int) -> List[int]:
        ...
        if not 0 <= center_block < self.n_clusters:
            raise IndexError(f"center_block {center_block} is outside the cluster range")
        out: List[int] = []
    ...
    def _cluster_candidates_from_lon_corridor(
        ...
        if not 0 <= block_idx < self.n_clusters:
            raise IndexError(f"block_idx {block_idx} is outside the cluster range")
        ...
        if self.corridor_anchor_mode == "width":
            query = np.array(
                [target_lat, np.clip(corridor_mid, lon_min, lon_max)], dtype=np.float64
            )
            _, idx = self.cluster_all_tree.query(query, k=k_anchor)
            idx = np.asarray(idx, dtype=np.int64).reshape(-1)
    ...
    def _point_to_segment_distance_sq(
        ...
        """Squared Euclidean distance from each point to a line segment."""
        segment = end - start
    ...
    def _vector_corridor_block_span(self, direction: np.ndarray) -> float:
        """Projected rectangular block width along a corridor direction."""
        if (
        ...
        return abs(float(direction[0])) * lat_width + abs(float(direction[1])) * lon_width
    ...
    def _cluster_candidates_from_vector_corridor(
        ...
        """Choose lagged blocks along ``self.past_step_vector``.

        Subclasses define the signed past-point offset and may override
        :meth:`_vector_corridor_block_span` to select a different footprint
        convention.  Candidate ranking and tie-breaking remain shared.
        """
        if count <= 0:
            return []
        ...
        if not 0 <= block_idx < self.n_clusters:
            raise IndexError(f"block_idx {block_idx} is outside the cluster range")
        past_step = np.asarray(self.past_step_vector, dtype=np.float64)
        if past_step.shape != (2,) or not np.isfinite(past_step).all():
            raise ValueError("past_step_vector must contain two finite components")
        ...
        if not np.isfinite([lo, hi]).all():
            raise ValueError("corridor multiplier endpoints must be finite")
        start = target + lo * past_step
        end = target + hi * past_step
        ...
        if self.corridor_anchor_mode == "width":
            ...
            block_span = self._vector_corridor_block_span(direction)
            if np.isfinite(block_span) and block_span > 0:
                anchor_count = max(
                    1,
    ...
    def _precompute_message(self) -> str:
        return (
            "Pre-computing corridor Vecchia "
