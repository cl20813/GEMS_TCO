...
# --- Assume SpatioTemporalModel is defined exactly as you provided ---
# (Contains matern_cov_aniso_STABLE_log_reparam, etc.)

class VecchiaLikelihoodPeriodicST(SpatioTemporalModel):
    """
    Implements the Vecchia likelihood for a full spatio-temporal model.
    
    COVARIANCE: 7-parameter (sigma, range_lon, range_lat, advec_lon, advec_lat, beta, nugget)
    MEAN TREND: 5-parameter (intercept, lon, lat, sin(wt), cos(wt))
    """
    
    def __init__(self, smooth:float, input_map :Dict[str,Any], aggregated_data: torch.Tensor, nns_map:Dict[str,Any], mm_cond_number:int, nheads:int):
        super().__init__(smooth, input_map, aggregated_data, nns_map, mm_cond_number) 
        self.nheads = nheads
        self.size_per_hour = self.input_map[self.key_list[0]].shape[0]
        
        # --- Define the Period for the Sin/Cos Trend ---
        # Assuming 8 hours based on old context. Change if needed.
        self.T_PERIOD_HOURS = 8.0 
        self.OMEGA = (2.0 * np.pi) / self.T_PERIOD_HOURS

    def _get_time_covariates(self, time_values: torch.Tensor) -> torch.Tensor:
        """Calculates the Sin(wt) and Cos(wt) terms for a tensor of time values."""
        # time_values is expected to be (N, 1)
        time_float = time_values.to(torch.float64)
        sin_t = torch.sin(self.OMEGA * time_float)
        cos_t = torch.cos(self.OMEGA * time_float)
        return torch.cat((sin_t, cos_t), dim=1)

    def _build_conditioning_set(self, time_idx: int, index: int):
        """
        Helper function to build the spatio-temporal conditioning set data.
        (This function is unchanged from your reference, it is correct)
        """
        current_np = self.input_map[self.key_list[time_idx]]
        current_row = current_np[index].reshape(1, -1)
    ...
    # -------------------------------------------------------------------
    # METHOD 1: FULL LIKELIHOOD (for HEADS) - OVERRIDDEN
    # -------------------------------------------------------------------
    def full_likelihood_avg(self, params: torch.Tensor, input_data: torch.Tensor, y: torch.Tensor, covariance_function: Callable) -> torch.Tensor:
            """
            Calculates the AVERAGE NLL per data point.
            💥 REVISED TREND: Intercept + Lat + Lon + Sin(wt) + Cos(wt)
            """
            input_data = input_data.to(torch.float64)
            y = y.to(torch.float64)
            N = input_data.shape[0]
            ...
            # Use the full ST covariance function
            cov_matrix = covariance_function(params=params, y= input_data, x= input_data)
            ...
            # --- 💥 REVISED TREND CALCULATION (5 Parameters) ---
            locs_spatial = input_data[:,:2].to(torch.float64) # [lat, lon]
            time_values = input_data[:, 3:4].to(torch.float64) # [t]
            
            intercept = torch.ones(N, 1, device=self.device, dtype=torch.float64)
            locs_time_periodic = self._get_time_covariates(time_values) # [Sin, Cos]
            
            # locs = [1, lat, lon, sin(t), cos(t)]
            locs = torch.cat((intercept, locs_spatial, locs_time_periodic), dim=1) # (N, 5)
            # --- End Trend ---
            ...
            C_inv_X = torch.cholesky_solve(locs, L, upper=False)  # (N, 5)
            C_inv_y = torch.cholesky_solve(y_col, L, upper=False) # (N, 1)

            tmp1 = torch.matmul(locs.T, C_inv_X)       # (5, 5)
            tmp2 = torch.matmul(locs.T, C_inv_y)       # (5, 1)
            
            try:
                jitter_beta = torch.eye(tmp1.shape[0], device=self.device, dtype=torch.float64) * 1e-8
                beta = torch.linalg.solve(tmp1 + jitter_beta, tmp2) # Solves for [b0..b4]
            except torch.linalg.LinAlgError:
                return torch.tensor(torch.inf, device=locs.device, dtype=locs.dtype)
    ...
    # -------------------------------------------------------------------
    # METHOD 2: COV STRUCTURE SAVER (for Conditionals) - OVERRIDDEN
    # -------------------------------------------------------------------
    def cov_structure_saver(self, params: torch.Tensor, covariance_function: Callable) -> Dict[str,Any]:
            """
            Pre-computes structural components.
            💥 REVISED TREND: [1, lat, lon, sin(t), cos(t)]
            """
            cov_map = defaultdict(lambda: defaultdict(dict))
            cut_line= self.nheads
            key_list = list(self.input_map.keys())
            ...
            for time_idx in range(0, len(key_list)):
                for index in range(cut_line, self.size_per_hour):
                    ...
                    # aggregated_arr contains all data (y) and all coordinates (lat, lon, t)
                    aggregated_arr = self._build_conditioning_set(time_idx, index)
                    N_cond = aggregated_arr.shape[0]
                    
                    # --- 💥 REVISED TREND CALCULATION (5 Parameters) ---
                    locs_spatial = aggregated_arr[:, :2] # [lat, lon]
                    time_values = aggregated_arr[:, 3:4] # [t]
                    
                    intercept = torch.ones(N_cond, 1, device=self.device, dtype=torch.float64)
                    locs_time_periodic = self._get_time_covariates(time_values) # [Sin, Cos]
                    
                    # locs = [1, lat, lon, sin(t), cos(t)]
                    locs = torch.cat((intercept, locs_spatial, locs_time_periodic), dim=1) # (N_cond, 5)
                    # --- End Trend ---

                    cov_matrix = covariance_function(params=params, y= aggregated_arr, x= aggregated_arr )
                    
                    try:
                        jitter = torch.eye(N_cond, device=self.device, dtype=torch.float64) * 1e-6
                        L_full = torch.linalg.cholesky(cov_matrix + jitter)
                    except torch.linalg.LinAlgError:
                        continue
                        
                    C_inv_locs = torch.cholesky_solve(locs, L_full, upper=False)
                    tmp1 = torch.matmul(locs.T, C_inv_locs) # tmp1 is now (5, 5)
                    ...
                    cov_map[(time_idx,index)] = {
                        'tmp1': tmp1,                     # (5, 5) matrix
                        'L_full': L_full, 
                        'cov_ygivenx': cov_ygivenx, 
                        'cond_mean_tmp': cond_mean_tmp, 
                        'log_det': log_det, 
                        'locs': locs.clone(),             # (N_cond, 5) matrix
                    }
            return cov_map


    # -------------------------------------------------------------------
    # METHOD 3: VECCHIA FULL BATCH - (Unchanged, uses new components)
    # -------------------------------------------------------------------
    def vecchia_space_time_fullbatch(self, params: torch.Tensor, covariance_function: Callable, cov_map:Dict[str,Any]) -> torch.Tensor:
            """
            Calculates the TOTAL AVERAGE NLL.
            This function is unchanged from your reference, as it correctly
            uses the components built by the overridden methods above.
            """
            cut_line= self.nheads
            key_list = list(self.input_map.keys())
            if not key_list:
            ...
            N_head = heads.shape[0]
            if N_head > 0:
                # 💥 This now calls the OVERRIDDEN full_likelihood_avg
                head_nll_avg = self.full_likelihood_avg(params, heads, heads[:, 2], covariance_function)
                head_nll_sum = head_nll_avg * N_head
            else:
                head_nll_sum = torch.tensor(0.0, device=self.device, dtype=torch.float64)
            ...
            for time_idx in range(0,len(self.input_map)):
                for index in range(cut_line, self.size_per_hour):
                    ...
                    # 💥 Load all pre-computed components (now 5x5)
                    tmp1 = cov_map[map_key]['tmp1']
                    L_full = cov_map[map_key]['L_full']
                    locs = cov_map[map_key]['locs'] # (N_cond, 5)
                    cov_ygivenx = cov_map[map_key]['cov_ygivenx']
                    cond_mean_tmp = cov_map[map_key]['cond_mean_tmp']
                    log_det = cov_map[map_key]['log_det']
                    ...
                    # This part remains the same, but matrices are (5,5), (5,1), etc.
                    C_inv_y = torch.cholesky_solve(aggregated_y.unsqueeze(-1), L_full, upper=False)
                    tmp2 = torch.matmul(locs.T, C_inv_y) # (5, 1)
                    
                    try:
                        jitter_beta = torch.eye(tmp1.shape[0], device=self.device, dtype=torch.float64) * 1e-8
                        beta = torch.linalg.solve(tmp1 + jitter_beta, tmp2) # Solves (5, 5) system
                    except torch.linalg.LinAlgError:
                        continue 
...
class fit_vecchia_lbfgs_ST(VecchiaLikelihoodPeriodicST): # 💥 INHERITS FROM NEW CLASS
    """
    Alternative fitting class that uses the L-BFGS optimizer.
    """
    ...
    def _convert_raw_params_to_interpretable(self, raw_params_list: List[float]) -> Dict[str, float]:
        """Converts the 7 raw optimized parameters back to interpretable model parameters."""
        # This function is unchanged from your reference and is correct
        try:
            log_phi1, log_phi2, log_phi3, log_phi4, advec_lat, advec_lon, log_nugget = raw_params_list
    ...
    def fit_vecc_lbfgs(self, 
        ...
        def closure():
            ...
            # 💥 This now calls the OVERRIDDEN cov_structure_saver
            cov_map = self.cov_structure_saver(params, covariance_function)
            # 💥 This now calls the OVERRIDDEN compute_vecc_nll
            loss = self.compute_vecc_nll(params, covariance_function, cov_map)
