...
class VecchiaBatched(SpatioTemporalModel):
    ...
    def precompute_conditioning_sets(self):
            ...
            unique_lons_desc = np.sort(unique_lons)[::-1]
            
            # 3. Select the Top 3 Longitudes
            if len(unique_lons_desc) < 3:
                raise ValueError(f"Grid only has {len(unique_lons_desc)} columns, cannot select 3.")
            target_lons = unique_lons_desc[:3] 
            
            # 4. Create Mask
            head_mask = np.isin(coords[:, 1].round(5), target_lons)
            
            # 5. Split Indices
            head_indices = np.where(head_mask)[0]
            tail_indices = np.where(~head_mask)[0] 
            
            # 6. Update nheads automatically
            self.nheads = len(head_indices)
            
            # -----------------------------------------------------------

            # --- 3. DEFINE GEOMETRIC OFFSETS ---
            # We look "Backward" at the Past (North and East).
            lat_step = 0.044
            lon_step = 0.063
            ...
            # 4. HEADS DATA (Extract using head_indices)
            heads_list = []
            for key in key_list:
                day_data = self.input_map[key]
                if isinstance(day_data, np.ndarray):
                    head_chunk = torch.from_numpy(day_data[head_indices]).to(self.device)
                else:
                    head_chunk = day_data[head_indices].clone().detach().to(self.device)
                heads_list.append(head_chunk)
            self.Heads_data = torch.cat(heads_list, dim=0).to(torch.float64)

            # 5. TAILS DATA (Task List using tail_indices)
            tasks = []
