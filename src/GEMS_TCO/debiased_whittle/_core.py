"""Shared numerical core for the configured Debiased Whittle engine.

This module preserves the established tapering, Fourier, covariance,
likelihood, parameter, dtype/device, jitter, and optimizer code.  Filter-only
differences are selected through an immutable ``SpatialFilterSpec``.

It is private by design.  Use :mod:`GEMS_TCO.debiased_whittle.engine` for new
code or the historical ``debiased_whittle_*`` compatibility modules.
"""
...
from GEMS_TCO.debiased_whittle.filters import SUMMED_FIRST_DIFFERENCES


class full_vecc_dw_likelihoods:
    """Shared DW/Vecchia comparison wrapper.

    The class attributes are overridden by the unified engine for each
    spatial-filter configuration.  Their defaults preserve the historical
    ``debiased_whittle_2110`` behavior.
    """
    ...
    def likelihood_wrapper(self,params, cov_fun, daily_aggregated_tensors_dw, daily_hourly_maps_dw):
        ...
        likelihood_class = self.likelihood_class or debiased_whittle_likelihood
        preprocess_class = self.preprocess_class or debiased_whittle_preprocess
        dwl = likelihood_class()
        ...
        db = preprocess_class(daily_aggregated_tensors_dw, daily_hourly_maps_dw, day_idx=0, params_list=self.params_list, lat_range=self.lat_range, lon_range=self.lon_range)
...
class debiased_whittle_preprocess(full_vecc_dw_likelihoods):
    """Preprocess a regular grid using the configured spatial filter."""
    ...
    def apply_first_difference_2d_tensor(self, df_tensor: torch.Tensor) -> torch.Tensor:
        """Apply the configured historical difference stencil.

        This compatibility method keeps the old public name.  New code should
        describe the operation through ``filter_spec.name`` instead.
        """
        return self.apply_spatial_filter(df_tensor)

    def apply_raw_passthrough(self, df_tensor: torch.Tensor) -> torch.Tensor:
        """Apply the historical ``raw`` preprocessing (demeaning included)."""
        return self.apply_spatial_filter(df_tensor)

    def apply_spatial_filter(self, df_tensor: torch.Tensor) -> torch.Tensor:
        """Apply exactly the stencil, crop, and demeaning in ``filter_spec``."""
        ...
        if spec.demean_input:
            # ``raw`` historically means no convolution, but it is not a
            # literal passthrough: every time slice is spatially demeaned over
            # observed (non-NaN) cells.  Preserve input row order and grid size.
            values = df_tensor[:, 2]
            valid_mask = ~torch.isnan(values)
            output = df_tensor.clone()
            if valid_mask.sum() > 0:
        ...
        if lat_count < shrink_lat + 1 or lon_count < shrink_lon + 1:
            # Deliberately retain the legacy default dtype/device on this edge
            # path; downstream characterization tests protect this behavior.
            return torch.empty(0, 4)

        sidx = (df_tensor[:, 0] * 1e6 + df_tensor[:, 1]).argsort()
        df_tensor = df_tensor[sidx]

        ozone_data = df_tensor[:, 2].reshape(1, 1, lat_count, lon_count)
        kernel_lat, kernel_lon = spec.kernel_shape
        diff_kernel = torch.zeros((1, 1, kernel_lat, kernel_lon), dtype=torch.float64)
        ...
        # PyTorch conv2d performs cross-correlation.  The stored stencils are
        # intentionally the unflipped coefficients used by the old modules.
    ...
    def generate_spatially_filtered_days(self,lat_s: float, lat_e: float, lon_s: float,  lon_e: float):
        ...
        if not tensors_to_aggregate:
            if self.filter_spec.demean_input:
                return torch.empty(0, 4, dtype=torch.float64)
            return torch.empty(0, 4)
...
class debiased_whittle_likelihood: # (full_vecc_dw_likelihoods):
    ...
    @classmethod
    def generate_Jvector_tapered(cls, tensor_list, tapering_func, lat_col, lon_col, val_col, device):
        """
        ...
        for tensor in tensor_list:
            data_grid, _ = cls._fill_grid_from_tensor(
    ...
    @classmethod
    def generate_Jvector_tapered_mv(cls, tensor_list, tapering_func, lat_col, lon_col, val_col, device):
        """
    ...
    @classmethod
    def cov_spatial_difference(cls, u1, u2, t, params, delta1, delta2):
        """
        ...
        if cls.filter_spec.demean_input:
            # Identity/raw uses a direct call in the historical implementation.
            # Do not express this as a one-weight double sum: keeping the direct
            # operation also preserves the original autograd graph.
            return cls.cov_x_spatiotemporal_model_kernel(u1_dev, u2_dev, t_dev, params)
        ...
        # Tuple order is part of the compatibility contract because it controls
        # floating-point accumulation order in the nested sum.
        ...
        for (a_idx, b_idx), w_ab in weights:
            ...
            for (c_idx, d_idx), w_cd in weights:
                ...
                term_cov = cls.cov_x_spatiotemporal_model_kernel(lag_u1, lag_u2, t_dev, params)
    ...
    @classmethod
    def cn_bar_tapered(cls, u1, u2, t, params, n1, n2, taper_autocorr_grid, delta1, delta2, q_idx=None, r_idx=None):
        """
        ...
        cov_X_value = cls.cov_spatial_difference(lag_u1, lag_u2, t_dev, params, delta1, delta2)
    ...
    @classmethod
    def expected_periodogram_fft_tapered(cls, params, n1, n2, p_time, taper_autocorr_grid, delta1, delta2):
        """
        ...
        for q in range(p_time):
            ...
            for r in range(p_time):
                ...
                term1 = cls.cn_bar_tapered(u1_mesh, u2_mesh, t_diff,
                                    params_tensor, n1, n2, taper_autocorr_grid, delta1, delta2, _q, _r)
                term2 = cls.cn_bar_tapered(u1_mesh - n1, u2_mesh, t_diff,
                                    params_tensor, n1, n2, taper_autocorr_grid, delta1, delta2, _q, _r)
                term3 = cls.cn_bar_tapered(u1_mesh, u2_mesh - n2, t_diff,
                                    params_tensor, n1, n2, taper_autocorr_grid, delta1, delta2, _q, _r)
                term4 = cls.cn_bar_tapered(u1_mesh - n1, u2_mesh - n2, t_diff,
                                    params_tensor, n1, n2, taper_autocorr_grid, delta1, delta2, _q, _r)
    ...
    def _retained_frequency_sum(cls, likelihood_terms, n1, n2):
        """Apply the legacy frequency mask and return ``(sum, count)``.

        The mask is explicit in :class:`SpatialFilterSpec`; it is not inferred
        from the stencil.  This preserves the historical likelihood for every
        filter, including the different row/column inclusion-exclusion rules.
        """
        ...
        if exclusion == "latitude_axis":
            row0_sum = likelihood_terms[0, :].sum() if n1 > 0 else torch.tensor(
                0.0, device=device, dtype=torch.float64
            )
            if torch.isnan(row0_sum).any() or torch.isinf(row0_sum).any():
                row0_sum = torch.tensor(0.0, device=device, dtype=torch.float64)
            sum_loss = total_sum - row0_sum if n1 > 1 else total_sum
            return sum_loss, n1 * n2 - n2

        if exclusion == "longitude_axis":
            col0_sum = likelihood_terms[:, 0].sum() if n2 > 0 else torch.tensor(
                0.0, device=device, dtype=torch.float64
            )
            if torch.isnan(col0_sum).any() or torch.isinf(col0_sum).any():
                col0_sum = torch.tensor(0.0, device=device, dtype=torch.float64)
            sum_loss = total_sum - col0_sum if n2 > 1 else total_sum
            return sum_loss, n1 * n2 - n1
        ...
        if exclusion == "both_axes":
            if n1 > 0 and n2 > 0:
                ...
                if torch.isnan(row0_sum + col0_sum - dc_term):
                    row0_sum = col0_sum = dc_term = torch.tensor(
                        0.0, device=device, dtype=torch.float64
                    )
                sum_loss = total_sum - row0_sum - col0_sum + dc_term
            else:
                sum_loss = total_sum
            return sum_loss, (n1 - 1) * (n2 - 1)

        # ``identity`` and ``summed_first_differences`` both retain every
        # spatial frequency except the DC point in the historical code.
        dc_term = likelihood_terms[0, 0] if n1 > 0 and n2 > 0 else torch.tensor(
            0.0, device=device, dtype=torch.float64
        )
    ...
    @classmethod
    def whittle_likelihood_loss_tapered(cls, params, I_sample, n1, n2, p_time, taper_autocorr_grid, delta1, delta2):
        """
        ...
        I_expected = cls.expected_periodogram_fft_tapered(
        ...
        sum_loss, num_terms = cls._retained_frequency_sum(likelihood_terms, n1, n2)
        if num_terms > 0:
    ...
    @classmethod
    def whittle_likelihood_loss_tapered_sum(cls, params, I_sample, n1, n2, p_time, taper_autocorr_grid, delta1, delta2):
        """
        ✅ Whittle Likelihood Loss (AVERAGED) using data tapering.
        """
        device = I_sample.device
        ...
        sum_loss, _ = cls._retained_frequency_sum(likelihood_terms, n1, n2)
    ...
    @classmethod
    def run_lbfgs_tapered(cls, params_list, optimizer, I_sample, n1, n2, p_time, taper_autocorr_grid, max_steps=5, device='cpu',grad_tol=1e-5):
        ...
        def closure():
            ...
            if cls.filter_spec.lbfgs_uses_sum_objective:
                # Preserve the historical 1111 closure exactly: request the
                # summed objective, then divide by the retained-frequency count.
                result = cls.whittle_likelihood_loss_tapered_sum(
                    params_tensor, I_sample_dev, n1, n2, p_time,
                    taper_autocorr_grid_dev, DELTA_LAT, DELTA_LON
                )
                if isinstance(result, tuple):
                    sum_loss, result_n1, result_n2 = result
                    n_terms = (result_n1 - 1) * (result_n2 - 1)
                    loss = sum_loss / n_terms if n_terms > 0 else sum_loss
                else:
                    loss = result
            else:
                loss = cls.whittle_likelihood_loss_tapered(
                    params_tensor, I_sample_dev, n1, n2, p_time,
                    taper_autocorr_grid_dev, DELTA_LAT, DELTA_LON
                )
        ...
        for i in range(max_steps):
            ...
            print(f'  Params (Raw Log): {cls.get_raw_log_params_7param(params_list)}')
        ...
        final_natural_params_str = cls.get_printable_params_7param(best_params_state)
        final_phi_params_str = cls.get_phi_params_7param(best_params_state)
        final_raw_params_str = cls.get_raw_log_params_7param(best_params_state)
