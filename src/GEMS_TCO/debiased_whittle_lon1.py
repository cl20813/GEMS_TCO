...
class debiased_whittle_preprocess(full_vecc_dw_likelihoods):
    ...
    def apply_first_difference_2d_tensor(self, df_tensor: torch.Tensor) -> torch.Tensor:
        """
        Applies 1D lon-direction first-difference filter.
        Z(i,j) = X(i,j+1) - X(i,j).  Output grid: M x (N-1).
        H(w1,w2) = e^{-iw2} - 1 → zero at entire w2=0 column.
        """
        ...
        # 3. Reshape data and define the correct kernel
        ozone_data = df_tensor[:, 2].reshape(1, 1, lat_count, lon_count)

        # Lon-only 1-1 difference: Z(i,j) = -X(i,j) + X(i,j+1)
        diff_kernel = torch.tensor([[[[-1., 1.]]]], dtype=torch.float64)
        ...
        # Lon shrinks by 1; lat stays full
        new_lats = unique_lats
...
class debiased_whittle_likelihood: # (full_vecc_dw_likelihoods):
    ...
    def cov_spatial_difference(u1, u2, t, params, delta1, delta2):
        ...
        """
        weights = {(0, 0): -1.0, (0, 1): 1.0}  # lon-only 1-1 difference
        device = params.device
    ...
    def whittle_likelihood_loss_tapered(params, I_sample, n1, n2, p_time, taper_autocorr_grid, delta1, delta2):
        ...
        # Exclude w2=0 column: H(w1,w2)=0 when w2=0 for lon-direction difference
        col0_sum = likelihood_terms[:, 0].sum() if n2 > 0 else torch.tensor(0.0, device=device, dtype=torch.float64)
        ...
        num_terms = n1 * n2 - n1  # exclude entire w2=0 column (n1 terms)
        if num_terms > 0:
