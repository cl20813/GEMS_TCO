...
class debiased_whittle_preprocess(full_vecc_dw_likelihoods):
    ...
    def apply_first_difference_2d_tensor(self, df_tensor: torch.Tensor) -> torch.Tensor:
        """
        Applies a 1D lat-only first-order difference filter.
        Z(i,j) = X(i+1,j) - X(i,j)   (lat direction only)

        Output grid: (nlat-1) x nlon  — lon dimension is unchanged.

        Motivation: spatial non-stationarity in the data is primarily a lat-direction
        gradient; differencing in lon is unnecessary and removes valid signal.
        """
        ...
        # 2. Sort by (lat, lon) before reshape to ensure correct grid ordering
        sidx = (df_tensor[:, 0] * 1e6 + df_tensor[:, 1]).argsort()
        df_tensor = df_tensor[sidx]

        # 3. Reshape and apply lat-only 1D diff kernel (2 x 1)
        # Z(i,j) = X(i+1,j) - X(i,j)
        ozone_data = df_tensor[:, 2].reshape(1, 1, lat_count, lon_count)
        diff_kernel = torch.tensor([[[[-1.], [1.]]]], dtype=torch.float64, device=df_tensor.device)

        # 3. Cross-correlation → valid output shape: (nlat-1, nlon)
        filtered_grid = F.conv2d(ozone_data, diff_kernel, padding='valid').squeeze()

        # Handle edge case: if filtered_grid is 1D (single row), unsqueeze
        if filtered_grid.dim() == 1:
            filtered_grid = filtered_grid.unsqueeze(0)

        # 4. Coordinates: lat shrinks by 1, lon unchanged
        new_lats = unique_lats[:-1]
        new_lons = unique_lons          # lon dimension preserved

        # 5. Reconstruct output tensor
...
class debiased_whittle_likelihood: # (full_vecc_dw_likelihoods):
    ...
    def cov_spatial_difference(u1, u2, t, params, delta1, delta2):
        ...
        where Y = X(s+delta1_lat) - X(s)  is the lat-only differenced field.

        Derivation:
          Cov_Y(u1,u2,t) = 2*C_X(u1,u2,t) - C_X(u1-delta1,u2,t) - C_X(u1+delta1,u2,t)

        Implemented via double-sum with filter weights h(0,0)=-1, h(1,0)=+1:
          Cov_Y = sum_{a,c in {0,1}} h_a * h_c * C_X(u1+(a-c)*delta1, u2, t)
        """
        weights = {(0, 0): -1.0, (1, 0): 1.0}   # lat-only 1D diff
        device = params.device
