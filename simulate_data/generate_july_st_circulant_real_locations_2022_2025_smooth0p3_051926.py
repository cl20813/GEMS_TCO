...
Smoothness-misspecification version: the data-generating process uses
Matérn smoothness nu=0.3, evaluated by a natural cubic spline.  The output
root is separate from the nu=0.5 baseline assets so downstream comparisons can
fit the same four Vecchia models to a deliberately rougher DGP.
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("/home/jl2815/tco/exercise_output/sim_data/july_st_circulant_realpattern_smooth0p3"))
