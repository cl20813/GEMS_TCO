...
Generalized-Cauchy version: the data-generating process uses generalized
Cauchy correlation with a=0.75, b=1.0 and nugget fixed exactly to 0.  The
output root is separate from the Matérn smooth0p3 assets, so downstream
correlation-model experiments can test this DGP without overwriting earlier
files.
...
def generalized_cauchy_corr(dist: torch.Tensor, a: float, b: float) -> torch.Tensor:
    a = float(a)
    b = float(b)
    if not (0.0 < a <= 2.0):
        raise ValueError(f"cauchy_a must be in (0, 2], got {a}")
    if b <= 0.0:
        raise ValueError(f"cauchy_b must be positive, got {b}")
    return torch.pow(1.0 + torch.pow(dist.clamp_min(0.0), a), -b / a)
...
def generate_st_field_block(
    ...
    cov = float(args.sigmasq) * generalized_cauchy_corr(
        dist,
        a=float(args.cauchy_a),
        b=float(args.cauchy_b),
    )
...
def process_year(year: int, args: argparse.Namespace, lats_hr: np.ndarray, lons_hr: np.ndarray, dlat_hr: float, dlon_hr: float) -> None:
    ...
    truth = {
        ...
        "cauchy_a": float(args.cauchy_a),
        "cauchy_b": float(args.cauchy_b),
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("/home/jl2815/tco/exercise_output/sim_data/july_st_circulant_realpattern_gencauchy_a0p75_b1p0_nugget0"))
    ...
    parser.add_argument("--cauchy-a", type=float, default=TRUE_DEFAULTS["cauchy_a"])
    parser.add_argument("--cauchy-b", type=float, default=TRUE_DEFAULTS["cauchy_b"])
