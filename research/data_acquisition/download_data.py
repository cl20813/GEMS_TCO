...
class Download_file:
    ...
    '''
    def __init__(self, year:int, month:int, days:list, ouptut_folder, api_key=None):
        self.year = year
        self.month = month
        self.days = days
        ...
        self.api_key = api_key or os.environ.get("GEMS_TCO_API_KEY")
    ...
    def run(self):
        if not self.api_key:
            raise ValueError(
                "Pass api_key or set the GEMS_TCO_API_KEY environment variable."
            )
