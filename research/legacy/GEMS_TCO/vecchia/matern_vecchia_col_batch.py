...
class ReverseLColumnVecchiaFitBatch:
    ...
    def precompute_conditioning_sets(self):
        ...
        if m_sizes:
            ...
            m_msg = f"m mean/med/max={m_arr.mean():.4f}/{np.median(m_arr):.4f}/{m_arr.max()}"
        else:
            m_msg = "m empty"
        print(
            f"Done in {time.time() - t0:.4f}s. grid={self._n_lat}x{self._n_lon}, "
