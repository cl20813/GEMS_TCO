"""
matern_vecchia_cluster_hybrid.py
  ...
  3. For a target block i at time t, use:
       ...
       C(t-2) = same block i
                ...
                + lag2_shifted_blocks fresh blocks around the two-step lag
                  center.  In the current candidate, lag1 and lag2 can be given
                  the same explicit lon offset, e.g. 0.126 and 0.126.
...
class ClusterHybridVecchiaFit(kernels_vecchia.fit_vecchia_lbfgs):
    ...
    def _conditioning_blocks(self, block_idx: int, time_idx: int) -> Tuple[str, int, List[Tuple[int, int]]]:
        ...
        if time_idx >= self.daily_stride:
            ...
            # C same: same spatial block at t-daily_stride.  With the usual
            # daily_stride=2 and 8 hourly snapshots per day asset, this is the
            # two-lag conditioning layer used by the pointwise hybrid model.
