...
def main():
    ...
    parser.add_argument('--bounds', type=float, nargs='+', default=[0.05, 100, 0.05, 100, 0.05, 20, 0.05, 20], help="Bounds for parameters" )
    parser.add_argument('--params', type=float,nargs='+', default=[0.5,0.5,0.5,0.5], help="sigmasq, range_, advec, beta, nugget ")
    ...
    coarse_map24_7, nns_map  = databyday_instance.process_forpurespace(coarse_map24_7, mm_cond_number)

    matern_spatial = kernels.matern_spatial()


    # data = grp[key]
    ####################################  data set up
    # reset keys for new map
    keys = sorted(coarse_map24_7)[0:key]
    with concurrent.futures.ThreadPoolExecutor(max_workers=32) as executor:
        futures = [
            executor.submit(
                matern_spatial.mle_parallel,
                key, bounds,initial_params, coarse_map24_7[key], mm_cond_number, nns_map
            )
            for key in keys 
        ]
