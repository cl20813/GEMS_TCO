...
def matern_corr(dist: torch.Tensor, smooth: float) -> torch.Tensor:
    if smooth == 0.5:
        return torch.exp(-dist)
    if smooth == 1.5:
        return (1.0 + dist) * torch.exp(-dist)
    raise ValueError("This generator supports smooth 0.5 or 1.5.")
...
def generate_st_field_block(
    ...
    cov = float(args.sigmasq) * matern_corr(dist, float(args.smooth))
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("/home/jl2815/tco/exercise_output/sim_data/july_st_circulant_realpattern"))
    ...
    parser.add_argument("--smooth", type=float, default=0.5, choices=[0.5, 1.5])
