#!/usr/bin/env python3
"""July 2024 x1 slice/tile spline-smooth spectral diagnostic for Amarel.

This script keeps the spectral machinery from
real_july_spline_smooth_spectral_maxmin_060726.py, but changes the experiment:

  year:           2024 by default
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="x1 slice/tile July real-data spectral diagnostics.")
    ...
    p.add_argument("--output-root", default="/home/jl2815/tco/exercise_output/summer/real_data/spline_smooth_spectral_x1_slice_tile_060926")
...
def plot_combined_domain_data_expected(
    ...
    plot_path = out_dir / f"{year}{args.month:02d}_{group}_{variant}_data_vs_expected_{profile}.png"
    fig.savefig(plot_path, dpi=180, bbox_inches="tight")
    plt.close(fig)
    curve_csv = out_dir / f"{year}{args.month:02d}_{group}_{variant}_data_vs_expected_{profile}.csv"
    write_csv(pd.concat(monthly_frames, ignore_index=True), curve_csv)
    if ratio_frames:
        ratio_csv = out_dir / f"{year}{args.month:02d}_{group}_{variant}_ratio_{profile}.csv"
...
def plot_combined_domain_expected_continuous(
    ...
    out_dir = combined_output_dir(args, smooth, year)
    out_dir.mkdir(parents=True, exist_ok=True)
    plot_path = out_dir / f"{year}{args.month:02d}_{group}_{variant}_expected_vs_continuous_{profile}.png"
    fig.savefig(plot_path, dpi=180, bbox_inches="tight")
    plt.close(fig)
    scaled_csv = out_dir / f"{year}{args.month:02d}_{group}_{variant}_expected_vs_continuous_{profile}.csv"
