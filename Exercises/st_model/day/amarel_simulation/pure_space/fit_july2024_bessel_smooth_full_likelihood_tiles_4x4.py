...
try:
    from GEMS_TCO.torch_bessel_full_likelihood import (
        ...
        raw_from_natural_np as torch_raw_from_natural_np,
    )

    TORCH_FULL_AVAILABLE = True
    TORCH_FULL_IMPORT_ERROR = ""
except Exception as exc:
    fit_full_matern_torch = None
    torch_raw_from_natural_np = None
    TORCH_FULL_AVAILABLE = False
    TORCH_FULL_IMPORT_ERROR = repr(exc)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--full-fit-engine", choices=["torch", "scipy"], default=os.environ.get("FULL_FIT_ENGINE", "torch"),
                   help="Final dense full-likelihood optimizer. torch estimates smooth by autograd; scipy uses scipy.minimize.")
    p.add_argument(
...
def _fit_full_tile_once(y: np.ndarray, coords: np.ndarray, task: dict) -> dict:
    engine = str(task.get("full_fit_engine", "torch")).lower()
    if engine == "torch":
        if not TORCH_FULL_AVAILABLE or fit_full_matern_torch is None or torch_raw_from_natural_np is None:
            raise RuntimeError(f"torch full likelihood unavailable: {TORCH_FULL_IMPORT_ERROR}")
        var_y = max(float(np.nanvar(y, ddof=1)), 1e-8)
        nugget_init = task["nugget_init"]
        if nugget_init is None:
            nugget_init = 0.20 * var_y
        start_raw = torch_raw_from_natural_np(
            sigmasq=var_y,
            range_lat=float(task["range_lat_init"]),
            range_lon=float(task["range_lon_init"]),
            smooth=float(task["smooth_init"]),
            nugget=float(nugget_init),
            nugget_mode=str(task["nugget_mode"]),
            smooth_bounds=tuple(task["smooth_bounds"]),
        )
        range_min, range_max = map(float, task["range_bounds"])
        sigmasq_min = 1e-12
        sigmasq_max = max(var_y * 1e5, 1e-6)
        nugget_min = max(var_y * 1e-8, 1e-10)
        nugget_max = max(var_y * 1e4, 1e-6)
        bounds = {
            "log_phi1": (
                math.log(sigmasq_min / range_max),
                math.log(sigmasq_max / range_min),
            ),
            "log_phi2": (math.log(1.0 / range_max), math.log(1.0 / range_min)),
            "log_phi3": (
                math.log((range_min / range_max) ** 2),
                math.log((range_max / range_min) ** 2),
            ),
            "smooth_raw": (-12.0, 12.0),
            "log_nugget": (math.log(nugget_min), math.log(nugget_max)),
        }
        fit = fit_full_matern_torch(
            ...
            mean_design=str(task["mean_design"]),
            jitter=float(task["jitter"]),
            max_iter=int(task["maxiter"]),
            max_eval=int(task["maxfun"]) if int(task["maxfun"]) > 0 else None,
            history_size=int(task["maxcor"]),
            bounds=bounds,
        )
        fit["nll"] = float(fit["loss"]) * int(len(y)) if np.isfinite(fit.get("loss", np.nan)) else np.nan
        fit["n_restarts"] = 1
        fit["fit_engine"] = "torch_full_bessel_autograd"
        fit["fit_method"] = METHOD
        return fit
...
def fit_tiles_for_hour(coords: np.ndarray, values: np.ndarray, tile_id: np.ndarray, tile_meta: dict, args: argparse.Namespace) -> pd.DataFrame:
    ...
    for tid in range(tile_count):
        ...
        tasks.append({
            ...
            "maxcor": int(args.maxcor),
            "optimizer_method": str(args.optimizer_method),
            "full_fit_engine": str(args.full_fit_engine),
