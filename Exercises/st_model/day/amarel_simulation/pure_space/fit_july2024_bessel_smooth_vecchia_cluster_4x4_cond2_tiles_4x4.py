...
This is the Vecchia counterpart to the full-likelihood 4x4 tile experiment.
Each hour is split into 4x4 tiles; inside each tile, targets are 4x4 grid-cell
...
from fit_july2024_bessel_smooth_full_likelihood_tiles_4x4 import (
    assign_tiles,
    choose_hour,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="July 2024 4x4 tile Bessel-smooth 4x4-cond2 Vecchia fits.")
    ...
    p.add_argument("--output-dir", default=os.environ.get(
        "OUTDIR",
        "/home/jl2815/tco/exercise_output/summer/july2024_bessel_smooth/vecchia_cluster_4x4_cond2_tiles_4x4",
    ))
...
def main() -> None:
    args = parse_args()
    if int(args.tile_y) != 4 or int(args.tile_x) != 4:
        print(f"WARNING: expected 4x4 tiles, got {args.tile_y}x{args.tile_x}")
