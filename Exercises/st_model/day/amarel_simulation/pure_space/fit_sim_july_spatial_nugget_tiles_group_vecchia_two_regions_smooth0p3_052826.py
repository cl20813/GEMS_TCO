...
conditioning sets by default.  It is used by the two-region July open-water
Slurm workflow for:

  * West Pacific:             lat [15, 20], lon [123, 145]
  * Micronesia open water:    lat [1, 6], lon [129, 139]

The Slurm workflow fixes Matern smoothness at nu=0.3 and separates outputs by
region.  The default mean design is beta0 + beta1 * centered latitude; this
removes the dominant meridional trend while keeping each hourly pure-space fit
separate.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default=os.environ.get("OUTDIR", "results/july_nugget_4x8_two_regions_smooth0p3"))
    ...
    parser.add_argument("--smooth", type=float, default=float(os.environ.get("SMOOTH", "0.3")))
    ...
    parser.add_argument("--lat-range", type=parse_float_pair, default=parse_float_pair(os.environ.get("LAT_RANGE", "15,20")),
                        help="Required fitted latitude range; pass per-region values in the Slurm loop.")
    parser.add_argument("--lon-range", type=parse_float_pair, default=parse_float_pair(os.environ.get("LON_RANGE", "123,145")),
                        help="Required fitted longitude range; pass per-region values in the Slurm loop.")
...
def heatmap_matrix(summary: pd.DataFrame, value_col: str, tile_y_count: int, tile_x_count: int) -> np.ndarray:
    mat = np.full((tile_y_count, tile_x_count), np.nan)
    for _, row in summary.iterrows():
        y_idx = int(round(float(row["tile_y"])))
        x_idx = int(round(float(row["tile_x"])))
        if 0 <= y_idx < tile_y_count and 0 <= x_idx < tile_x_count:
            mat[y_idx, x_idx] = row[value_col]
    return mat
...
def main() -> None:
    ...
    if not np.isclose(smooth, 0.3):
        raise SystemExit(f"ERROR: this two-region entrypoint is fixed to --smooth 0.3, got {smooth}.")
    try:
