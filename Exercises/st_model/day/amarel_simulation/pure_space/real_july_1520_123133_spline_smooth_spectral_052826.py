#!/usr/bin/env python3
"""July real-data spline-smooth spectral diagnostic for 1520_123133.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Spline-smooth July real-data spectral diagnostics for 1520_123133.")
    ...
    p.add_argument("--smooths", default="0.2,0.3,0.5,1")
    ...
    p.add_argument("--output-root", default="/home/jl2815/tco/exercise_output/real_data/real_july_1520_123133_spline_smooth_spectral_052826")
    p.add_argument("--expanded-bounds", action=argparse.BooleanOptionalAction, default=True)
    p.add_argument("--lat-range", default="15,20")
    p.add_argument("--lon-range", default="123,133")
...
def load_month_pickle(args: argparse.Namespace, year: int, lat_range: tuple[float, float], lon_range: tuple[float, float]):
    ...
    if args.expanded_bounds:
        ...
        if tuple(lat_range) == (15.0, 20.0) and tuple(lon_range) == (123.0, 133.0):
            candidates.append(month_dir / expanded_grid_filename(year, args.month, (15.0, 20.0), (123.0, 145.0)))
...
def make_monthly_average(args: argparse.Namespace, ctx: MonthContext, smooth: float, out_dir: Path):
    ...
    quicklook_dir = Path(args.output_root) / "monthly_average_plots"
    quicklook_dir.mkdir(parents=True, exist_ok=True)
    quicklook_path = quicklook_dir / (
        f"real_july_1520_123133_{ctx.year}{ctx.month:02d}_"
        f"smooth{smooth_tag(smooth)}_30day_mean_data_vs_expected_periodogram.png"
    )
    shutil.copy2(plot_path, quicklook_path)
    print(f"Copied monthly average plot: {quicklook_path}", flush=True)
