...
for path in [HERE, SPACE_TIME_DIR, REAL_DATA_DIR]:
...
from st_corridor_common import (
...
RATIO_PANEL_SPECS = [
    (
        "ratio_I_over_EI_profile_mean",
        "I / E[I]",
        "marginal spatial I / diagonal E[I], profile sigma",
        ("ratio_I_over_EI_profile_p10", "ratio_I_over_EI_profile_p90"),
    ),
    (
        "ratio_EI_over_continuous_mean",
        "E[I] / continuous",
        "finite-sample diagonal E[I] / continuous-like spectrum",
        ("ratio_EI_over_continuous_p10", "ratio_EI_over_continuous_p90"),
    ),
    (
        "whitened_ratio_mean",
        "8x8 whitened I / E[I]",
        "8x8 Cholesky-whitened quadratic power / profiled target",
        ("whitened_ratio_p10", "whitened_ratio_p90"),
    ),
]
...
def refresh_outputs(out_dir: Path, fit_rows: list[dict[str, Any]], profile_rows: list[dict[str, Any]], top_plot_dir: Path | None) -> None:
    ...
    if not profile_df.empty:
        ...
        plot_ratio_triptychs_by_year_direction(monthly, out_dir / "monthly_average_plots")
        plot_directional_year_outputs(monthly, out_dir / "monthly_average_plots")
        plot_daily_norm_ratio_outputs(profile_df, out_dir)
        if top_plot_dir is not None:
            top_plot_dir.mkdir(parents=True, exist_ok=True)
            plot_ratio_triptychs_by_year_direction(monthly, top_plot_dir)
            plot_directional_year_outputs(monthly, top_plot_dir)
            plot_daily_norm_ratio_outputs(profile_df, top_plot_dir)
...
def plot_ratio_triptych(sub: pd.DataFrame, path: Path, title: str, x_label: str) -> None:
    if sub.empty:
        return
    fig, axes = plt.subplots(1, 3, figsize=(16, 4.8), sharex=True, sharey=True)
    for ax, (metric, short_title, ylabel, band_cols) in zip(axes, RATIO_PANEL_SPECS):
        if metric not in sub.columns:
            ax.axis("off")
            continue
        for model_variant, sub_model in sub.groupby("model_variant", dropna=False):
            sub_model = sub_model.sort_values("k_mid")
            label = model_label_with_loss(sub_model)
            x = pd.to_numeric(sub_model["k_mid"], errors="coerce").to_numpy(dtype=float)
            y = pd.to_numeric(sub_model[metric], errors="coerce").to_numpy(dtype=float)
            ok = np.isfinite(x) & np.isfinite(y) & (y > 0)
            if not ok.any():
                continue
            line = ax.plot(x[ok], y[ok], linewidth=1.9, label=label)[0]
            lo_col, hi_col = band_cols
            if lo_col in sub_model.columns and hi_col in sub_model.columns:
                lo = pd.to_numeric(sub_model[lo_col], errors="coerce").to_numpy(dtype=float)
                hi = pd.to_numeric(sub_model[hi_col], errors="coerce").to_numpy(dtype=float)
                ok_band = ok & np.isfinite(lo) & np.isfinite(hi) & (lo > 0) & (hi > 0)
                if ok_band.any():
                    ax.fill_between(x[ok_band], lo[ok_band], hi[ok_band], color=line.get_color(), alpha=0.14, linewidth=0)
        ax.axhline(1.0, color="0.25", linestyle="--", linewidth=1.0)
...
def plot_ratio_triptychs_by_year_direction(monthly: pd.DataFrame, base_dir: Path) -> None:
    if monthly.empty:
        return
    for year, sub_year in monthly.groupby("year", dropna=False):
        year_dir = base_dir / f"year_{int(year)}"
        year_dir.mkdir(parents=True, exist_ok=True)
        for direction in ["norm", "lat", "lon", "diag"]:
            sub = sub_year[sub_year["direction"] == direction].copy()
            if sub.empty:
                continue
            plot_ratio_triptych(
                sub,
                year_dir / f"ratio_triptych_{direction}.png",
                title=f"Real July {int(year)}: ST corridor spectral ratio diagnostics, {direction_title(direction)}",
                x_label=direction_xlabel(direction),
            )


def plot_daily_norm_ratio_outputs(profile: pd.DataFrame, base_dir: Path) -> None:
    if profile.empty or "direction" not in profile.columns:
        return
    norm = profile[profile["direction"] == "norm"].copy()
    if norm.empty:
        return
    keep_cols = [
        "year",
        ...
        "k_mid",
        "ratio_I_over_EI_profile_mean",
        "ratio_I_over_EI_profile_p10",
        "ratio_I_over_EI_profile_p90",
        "ratio_I_over_EI_mean",
        "global_scale",
        "profile_sigmasq",
    ]
    for year, sub_year in norm.groupby("year", dropna=False):
        year_dir = base_dir / f"year_{int(year)}" / "daily_norm_ratio_plots"
        year_dir.mkdir(parents=True, exist_ok=True)
        save_rows(year_dir / "daily_norm_I_over_EI_profile_ratio_rows.csv", sub_year[[c for c in keep_cols if c in sub_year.columns]])
        for (day_idx, day), sub_day in sub_year.groupby(["day_idx", "day"], dropna=False):
            fig, ax = plt.subplots(figsize=(8.5, 5.2))
            for model_variant, sub_model in sub_day.groupby("model_variant", dropna=False):
                sub_model = sub_model.sort_values("k_mid")
                label = model_label_with_loss(sub_model)
                x = pd.to_numeric(sub_model["k_mid"], errors="coerce").to_numpy(dtype=float)
                y = pd.to_numeric(sub_model["ratio_I_over_EI_profile_mean"], errors="coerce").to_numpy(dtype=float)
                ok = np.isfinite(x) & np.isfinite(y) & (y > 0)
                if not ok.any():
                    continue
                ax.plot(x[ok], y[ok], linewidth=1.8, label=label)
            ax.axhline(1.0, color="0.25", linestyle="--", linewidth=1.0)
            ax.set_title(f"Real July {int(year)} day_idx={int(day_idx)} ({day}): norm-frequency I / E[I]")
            ax.set_xlabel("norm frequency")
            ax.set_ylabel("marginal spatial I / diagonal E[I], profile sigma")
            ax.set_yscale("log")
            ax.set_ylim(0.2, 5.0)
            ax.grid(alpha=0.25, which="both")
            ax.legend(fontsize=8)
            fig.tight_layout()
            fig.savefig(year_dir / f"dayidx_{int(day_idx):02d}_norm_I_over_EI_profile_ratio.png", dpi=180, bbox_inches="tight")
            plt.close(fig)
...
def model_label_with_loss(sub_model: pd.DataFrame) -> str:
    ...
    if "loss_median" in sub_model.columns:
        ...
        loss_source = sub_model["loss"]
    else:
        loss_source = pd.Series(np.nan, index=sub_model.index)
    loss = float(pd.to_numeric(loss_source, errors="coerce").median())
    if np.isfinite(loss):
...
def filter_rows_for_requested_variants(rows: list[dict[str, Any]], years: list[int], requested_variants: list[str]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if year not in year_set:
            continue
        if str(row.get("model_variant")) not in variants_for_year(year, requested_variants):
            continue
        out.append(row)
    return out
...
def main() -> None:
    ...
    if args.skip_existing:
        fit_rows = filter_rows_for_requested_variants(fit_rows, years, model_variants)
        profile_rows = filter_rows_for_requested_variants(profile_rows, years, model_variants)
    ...
    print("directional monthly ratio plots:", out_dir / "monthly_average_plots", flush=True)
    print("daily norm-frequency ratio plots:", out_dir / "year_<year>" / "daily_norm_ratio_plots", flush=True)
