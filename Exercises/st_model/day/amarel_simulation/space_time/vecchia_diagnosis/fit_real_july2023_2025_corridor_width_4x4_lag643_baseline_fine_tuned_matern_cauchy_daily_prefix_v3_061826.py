#!/usr/bin/env python3
"""Real July 2023-2025 ST corridor Vecchia baseline-vs-fine-tuned diagnosis v3.

This is the daily-density comparison requested after the v2 monthly-average run:

  - baseline: Vecchia lag 6/4/3, 4x4=16-cell block geometry,
    Matérn with fixed smooth=0.3 and nugget fixed at zero;
  - fine_tuned candidates: day-specific generalized Cauchy beta values from
    the tuning table, with alpha swept over 0.75, 0.8, 0.9, and 1.0;
  - the same 4x4 corridor lag-643 Vecchia geometry;
  - max-min block-prefix fits at 100, 200, 400, 600, 800, and all blocks;
  - real July 2023, 2024, and 2025 data, days 1-30 by default.

The output includes final Vecchia objective values from each fitted model.
The Vecchia engine already returns a per-observation objective; the retained
``loss_per_valid`` output column is therefore an alias kept for compatibility.
Monthly plots remain, and daily plots are added for each year/day to show how
estimates behave as the block-prefix density grows.
"""
...
GC_ALPHA_SWEEP = [0.75, 0.8, 0.9, 1.0]
GC_BETA_VALUES = [0.5, 1.0, 3.0, 4.0]
GC_DAY_TABLE_SWEEP_TOKEN = "gc_day_table_sweep"
...
def gc_variant_name(alpha: float, beta: float) -> str:
    return f"gc_a{value_tag(alpha)}_b{value_tag(beta)}"


def gc_label(alpha: float, beta: float) -> str:
    return f"GC a={float(alpha):g} b={float(beta):g}"


def gc_spec(alpha: float, beta: float, role: str = "fine_tuned") -> dict[str, Any]:
    return {
        "family": "cauchy",
        "smooth": np.nan,
        "gc_alpha": float(alpha),
        "gc_beta": float(beta),
        "label": gc_label(alpha, beta),
        "role": role,
    }
...
VARIANT_SPECS: dict[str, dict[str, Any]] = {
    "matern_s03": {
        ...
        "gc_alpha": np.nan,
        "gc_beta": np.nan,
        "label": "Baseline Matern s=0.3",
        "role": "baseline",
    },
    "fine_tuned_matern_s03": {
        "family": "matern",
        "smooth": 0.3,
        "gc_alpha": np.nan,
        "gc_beta": np.nan,
        "label": "Fine tuned Matern s=0.3",
        "role": "fine_tuned",
    },
}

for _alpha in GC_ALPHA_SWEEP:
    for _beta in GC_BETA_VALUES:
        VARIANT_SPECS[gc_variant_name(_alpha, _beta)] = gc_spec(_alpha, _beta)


YEAR_FALLBACK_GC_BETA: dict[int, float] = {
    2023: 0.5,
    2024: 0.5,
    2025: 0.5,
}

YEAR_DEFAULT_GC_ALPHA: dict[int, float] = {
    2023: 0.75,
    2024: 0.8,
    2025: 0.75,
}

TUNED_MODEL_BY_DAY: dict[tuple[int, int], dict[str, Any]] = {
    (2023, 0): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 1): {"family": "cauchy", "alpha": 0.75, "beta": 4.0},
    (2023, 2): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 3): {"family": "cauchy", "alpha": 0.75, "beta": 4.0},
    (2023, 4): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 5): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 6): {"family": "matern", "smooth": 0.3},
    (2023, 7): {"family": "cauchy", "alpha": 0.75, "beta": 1.0},
    (2023, 8): {"family": "matern", "smooth": 0.3},
    (2023, 9): {"family": "matern", "smooth": 0.3},
    (2023, 10): {"family": "cauchy", "alpha": 0.75, "beta": 1.0},
    (2023, 11): {"family": "cauchy", "alpha": 0.75, "beta": 3.0},
    (2023, 12): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 13): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 14): {"family": "matern", "smooth": 0.3},
    (2023, 15): {"family": "cauchy", "alpha": 0.75, "beta": 1.0},
    (2023, 16): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 17): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 18): {"family": "cauchy", "alpha": 0.75, "beta": 1.0},
    (2023, 19): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 20): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 21): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 22): {"family": "cauchy", "alpha": 0.75, "beta": 1.0},
    (2023, 23): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 24): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 25): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 26): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 27): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 28): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2023, 29): {"family": "matern", "smooth": 0.3},
    (2024, 0): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 1): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 2): {"family": "cauchy", "alpha": 0.8, "beta": 3.0},
    (2024, 3): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 4): {"family": "cauchy", "alpha": 0.8, "beta": 3.0},
    (2024, 5): {"family": "matern", "smooth": 0.3},
    (2024, 6): {"family": "cauchy", "alpha": 0.8, "beta": 1.0},
    (2024, 7): {"family": "matern", "smooth": 0.3},
    (2024, 8): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 9): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 10): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 11): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 12): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 13): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 14): {"family": "cauchy", "alpha": 0.8, "beta": 4.0},
    (2024, 15): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 16): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 17): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 18): {"family": "cauchy", "alpha": 0.8, "beta": 1.0},
    (2024, 19): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 20): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 21): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 22): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 23): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 24): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 25): {"family": "cauchy", "alpha": 0.8, "beta": 1.0},
    (2024, 26): {"family": "cauchy", "alpha": 0.8, "beta": 1.0},
    (2024, 27): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2024, 28): {"family": "cauchy", "alpha": 0.8, "beta": 1.0},
    (2024, 29): {"family": "cauchy", "alpha": 0.8, "beta": 0.5},
    (2025, 0): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 1): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 2): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 3): {"family": "cauchy", "alpha": 0.75, "beta": 1.0},
    (2025, 4): {"family": "matern", "smooth": 0.3},
    (2025, 5): {"family": "cauchy", "alpha": 0.75, "beta": 3.0},
    (2025, 6): {"family": "cauchy", "alpha": 0.75, "beta": 4.0},
    (2025, 7): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 8): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 9): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 10): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 11): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 12): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 13): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 14): {"family": "cauchy", "alpha": 0.75, "beta": 4.0},
    (2025, 15): {"family": "cauchy", "alpha": 0.75, "beta": 1.0},
    (2025, 16): {"family": "matern", "smooth": 0.3},
    (2025, 17): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 18): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 19): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 20): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 21): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 22): {"family": "cauchy", "alpha": 0.75, "beta": 1.0},
    (2025, 23): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 24): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 25): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 26): {"family": "matern", "smooth": 0.3},
    (2025, 27): {"family": "matern", "smooth": 0.3},
    (2025, 28): {"family": "cauchy", "alpha": 0.75, "beta": 0.5},
    (2025, 29): {"family": "cauchy", "alpha": 0.75, "beta": 1.0},
}

DEFAULT_MODEL_VARIANTS = ["matern_s03", GC_DAY_TABLE_SWEEP_TOKEN]

RUN_STEM = "real_july2023_2025_corridor_lag643_baseline_fine_tuned_daily_prefix_v3_061826"
ALL_FITS_CSV = f"{RUN_STEM}_all_fits.csv"
PARAM_SUMMARY_CSV = f"{RUN_STEM}_monthly_param_summary.csv"
LOSS_SUMMARY_CSV = f"{RUN_STEM}_monthly_loss_summary.csv"
MISSING_CSV = f"{RUN_STEM}_missing.csv"
JSONL_NAME = f"{RUN_STEM}_all_fits.jsonl"
...
def variant_spec(name: str) -> dict[str, Any]:
    if name == GC_DAY_TABLE_SWEEP_TOKEN:
        return {
            "family": "meta",
            "smooth": np.nan,
            "gc_alpha": np.nan,
            "gc_beta": np.nan,
            "label": "Day-table GC alpha sweep",
            "role": "meta",
            "model_variant": name,
        }
...
def tuned_model_for_day(year: int, day_idx: int) -> dict[str, Any]:
    tuned = TUNED_MODEL_BY_DAY.get((int(year), int(day_idx)))
    if tuned is not None:
        return tuned
    alpha = YEAR_DEFAULT_GC_ALPHA.get(int(year), 0.75)
    beta = YEAR_FALLBACK_GC_BETA.get(int(year), 0.5)
    return {"family": "cauchy", "alpha": float(alpha), "beta": float(beta)}


def day_table_gc_beta(year: int, day_idx: int) -> float:
    tuned = tuned_model_for_day(int(year), int(day_idx))
    if str(tuned.get("family")) == "cauchy":
        return float(tuned["beta"])
    return float(YEAR_FALLBACK_GC_BETA.get(int(year), 0.5))


def expand_requested_variants_for_day(year: int, day_idx: int, requested_variants: list[str]) -> list[str]:
    out: list[str] = []
    for name in requested_variants:
        if name == GC_DAY_TABLE_SWEEP_TOKEN:
            beta = day_table_gc_beta(int(year), int(day_idx))
            out.extend(gc_variant_name(alpha, beta) for alpha in GC_ALPHA_SWEEP)
        else:
            out.append(name)
...
def fit_one(
    ...
    row = {
        ...
        "model_role": str(spec.get("role", "fine_tuned")),
...
def model_style(model_variant: str):
    styles = {
        ...
        "fine_tuned_matern_s03": ("tab:cyan", "D", "--"),
    }
    if str(model_variant) in styles:
        return styles[str(model_variant)]
    match = re.fullmatch(r"gc_a(?P<alpha>[0-9p]+)_b(?P<beta>[0-9p]+)", str(model_variant))
    if match:
        alpha = float(match.group("alpha").replace("p", "."))
        beta = float(match.group("beta").replace("p", "."))
        color_by_beta = {0.5: "tab:orange", 1.0: "tab:green", 3.0: "tab:red", 4.0: "tab:purple"}
        marker_by_alpha = {0.75: "v", 0.8: "P", 0.9: "s", 1.0: "^"}
        return (
            color_by_beta.get(beta, "0.25"),
            marker_by_alpha.get(alpha, "o"),
            "--" if alpha in {0.9, 1.0} else "-",
        )
    return ("0.25", "o", "-")
...
def year_model_loss_lookup(loss_summary: pd.DataFrame | None) -> dict[tuple[int, str], float]:
    ...
    for (year, model_variant), sub in loss_summary.groupby(["year", "model_variant"], dropna=False):
        vals = pd.to_numeric(sub["loss_per_valid_mean"], errors="coerce").dropna().to_numpy(dtype=float)
        if vals.size:
...
def label_with_loss(base_label: str, loss_mean: float | None) -> str:
    if loss_mean is None or not np.isfinite(float(loss_mean)):
        return base_label
    return f"{base_label} loss/obs={float(loss_mean):.{LOSS_DECIMALS}f}"
...
def plot_loss_summary(loss_summary: pd.DataFrame, out_dir: Path, monthly_out_dir: Path | None = None) -> None:
    ...
    for year in sorted(pd.to_numeric(loss_summary["year"], errors="coerce").dropna().astype(int).unique()):
        ...
        metrics = [
            ("loss_per_valid_mean", "loss/obs mean"),
            ("loss_per_valid_median", "loss/obs median"),
    ...
    if not table.empty:
        ...
        ax.set_title("Median final Vecchia objective per observation, pooled across 2023-2025")
        for i in range(table.shape[0]):
            for j in range(table.shape[1]):
                val = arr[i, j]
                if np.isfinite(val):
                    ax.text(j, i, f"{val:.{LOSS_DECIMALS}f}", ha="center", va="center", color="white", fontsize=8)
...
def daily_plot_dirs(out_dir: Path, monthly_out_dir: Path | None = None) -> list[Path]:
    raw_dirs = [out_dir / "daily_plots"]
    if monthly_out_dir is not None:
        raw_dirs.append(monthly_out_dir.parent / "daily_plots")
    plot_dirs = list(dict.fromkeys(path.resolve() for path in raw_dirs))
    for plot_dir in plot_dirs:
        plot_dir.mkdir(parents=True, exist_ok=True)
    return plot_dirs


def daily_label_with_loss(label: str, sub_model: pd.DataFrame) -> str:
    vals = pd.to_numeric(sub_model["loss_per_valid"], errors="coerce").dropna().to_numpy(dtype=float)
    if vals.size == 0:
        return label
    return f"{label} loss/obs={float(np.mean(vals)):.{LOSS_DECIMALS}f}"


def plot_daily_param_loss(ok: pd.DataFrame, out_dir: Path, monthly_out_dir: Path | None = None) -> None:
    if ok.empty:
        return
    plot_dirs = daily_plot_dirs(out_dir, monthly_out_dir)
    ok = ok.copy()
    ok["year"] = pd.to_numeric(ok["year"], errors="coerce")
    ok["day_idx"] = pd.to_numeric(ok["day_idx"], errors="coerce")
    ok = ok.dropna(subset=["year", "day_idx"])
    if ok.empty:
        return

    for (year, day_idx), sub_day in ok.groupby(["year", "day_idx"], dropna=False):
        year_i = int(year)
        day_idx_i = int(day_idx)
        prefixes, labels = ordered_prefixes(sub_day)
        if not prefixes:
            continue
        day_label = str(sub_day["day"].dropna().iloc[0]) if sub_day["day"].notna().any() else f"{year_i}-07-{day_idx_i + 1:02d}"
        n_valid = pd.to_numeric(sub_day["n_valid_o3"], errors="coerce").dropna()
        n_valid_text = f"{int(n_valid.max())}" if not n_valid.empty else "NA"
        x_pos = np.arange(len(prefixes))
        fig, axes = plt.subplots(3, 3, figsize=(14, 10), sharex=True)
        axes_flat = axes.ravel()
        ...
        for ax, param in zip(axes_flat, P_LABELS):
            col = f"est_{param}"
            if col not in sub_day.columns:
                ax.axis("off")
                continue
            for model_variant, sub_model in sub_day.groupby("model_variant", dropna=False):
                sub_model = sub_model.set_index("block_prefix_requested").reindex(prefixes)
                y = pd.to_numeric(sub_model[col], errors="coerce").to_numpy(dtype=float)
                color, marker, ls = model_style(str(model_variant))
                base_label = str(sub_model["model_label"].dropna().iloc[0]) if sub_model["model_label"].notna().any() else str(model_variant)
                label = daily_label_with_loss(base_label, sub_model)
                finite = np.isfinite(y)
                ax.plot(x_pos[finite], y[finite], marker=marker, linestyle=ls, color=color, linewidth=1.7, label=label)
            ax.set_title(param)
            ax.set_xticks(x_pos)
        ...
        loss_ax = axes_flat[len(P_LABELS)]
        for model_variant, sub_model in sub_day.groupby("model_variant", dropna=False):
            sub_model = sub_model.set_index("block_prefix_requested").reindex(prefixes)
            y = pd.to_numeric(sub_model["loss_per_valid"], errors="coerce").to_numpy(dtype=float)
            color, marker, ls = model_style(str(model_variant))
            base_label = str(sub_model["model_label"].dropna().iloc[0]) if sub_model["model_label"].notna().any() else str(model_variant)
            label = daily_label_with_loss(base_label, sub_model)
            finite = np.isfinite(y)
            loss_ax.plot(x_pos[finite], y[finite], marker=marker, linestyle=ls, color=color, linewidth=1.9, label=label)
        loss_ax.set_title(f"loss/obs ({LOSS_DECIMALS} decimals)")
        loss_ax.set_xticks(x_pos)
        loss_ax.set_xticklabels(labels)
        loss_ax.yaxis.set_major_formatter(FuncFormatter(lambda x, _pos: f"{x:.{LOSS_DECIMALS}f}"))
        loss_ax.grid(alpha=0.25)

        for ax in axes_flat[len(P_LABELS) + 1 :]:
            ax.axis("off")
        axes_flat[0].legend(fontsize=6.5)
        fig.suptitle(
            f"Real July {day_label}: daily parameter/loss path by block-prefix density "
            f"(valid obs max={n_valid_text})"
        )
        fig.tight_layout()
        for plot_dir in plot_dirs:
            year_dir = plot_dir / f"year_{year_i}"
            year_dir.mkdir(parents=True, exist_ok=True)
            fig.savefig(
                year_dir / f"real_{year_i}_day{day_idx_i + 1:02d}_parameter_loss_by_blockmaxmin.png",
                dpi=180,
                bbox_inches="tight",
            )
        plt.close(fig)
...
def filter_current_requested_rows(df: pd.DataFrame, requested_model_variants: list[str]) -> pd.DataFrame:
    if df.empty or not {"year", "day_idx", "model_variant"}.issubset(df.columns):
        return df
    keep = []
    for _, row in df.iterrows():
        try:
            active = set(
                expand_requested_variants_for_day(
                    int(row["year"]),
                    int(row["day_idx"]),
            ...
            keep.append(False)
            continue
        keep.append(str(row["model_variant"]) in active)
...
def refresh_outputs(
    ...
    df = filter_current_requested_rows(raw_df, model_variants)
    df = save_rows(out_dir / ALL_FITS_CSV, df)
    ...
    plot_daily_param_loss(ok, out_dir, monthly_out_dir)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Real July 2023-2025 ST Vecchia baseline-vs-fine-tuned daily-density prefix test v3.")
    parser.add_argument("--real-years", nargs="+", default=["2023", "2024", "2025"])
    parser.add_argument("--month", type=int, default=7)
    parser.add_argument("--days", default="0,30", help="'0,30' means day_idx 0..29, i.e. July days 1-30; use '0,31' for all July.")
...
def load_existing(out_dir: Path) -> list[dict[str, Any]]:
    ...
    if "loss" in df.columns:
        df["loss_per_valid"] = pd.to_numeric(df["loss"], errors="coerce")
...
def main() -> None:
    ...
    print("gc_alpha_sweep:", GC_ALPHA_SWEEP, flush=True)
    print("year_fallback_gc_beta:", YEAR_FALLBACK_GC_BETA, flush=True)
    ...
    run_config = {
        ...
        "gc_alpha_sweep": GC_ALPHA_SWEEP,
        "gc_beta_values": GC_BETA_VALUES,
        "gc_day_table_sweep_token": GC_DAY_TABLE_SWEEP_TOKEN,
        "year_default_gc_alpha": YEAR_DEFAULT_GC_ALPHA,
        "year_fallback_gc_beta": YEAR_FALLBACK_GC_BETA,
        "tuned_model_by_day": TUNED_MODEL_BY_DAY,
        ...
        "comparison_definition": "baseline=matern_s03 Vecchia lag 6/4/3 with 4x4=16-cell blocks; fine_tuned=day-table generalized Cauchy beta with alpha swept over 0.75, 0.8, 0.9, 1.0",
    ...
    for asset in assets:
        asset_model_variants = expand_requested_variants_for_day(int(asset["year"]), int(asset["day_idx"]), model_variants)
