#!/usr/bin/env python3
"""One-day advection mismatch simulation for conditional eigen diagnostics.

This driver keeps the production conditional-eigen engine unchanged.  It
patches in fixed-advection Matern variants and, only when needed, generates a
single smooth=0.5 July ST simulated day using the reusable circulant generator.

Experiments:
  advection_mismatch:
    DGP smooth=0.5 nugget=0, fit smooth=0.5/nugget=0 with advection fixed at
    true, zero, and large values.
  advection_mismatch_dgp1:
    Same experiment for DGP nugget=1, matching the local smooth0p5 simulated
    asset used in the recent notebooks.
"""
...
MODEL_SPECS: dict[str, dict[str, Any]] = {
    "matern_s05_n0_advec_true": {
        ...
        "gc_alpha": np.nan,
        "gc_beta": np.nan,
        "label": "Matern s=0.5 nugget0 advec true",
        "color": "#2ca02c",
    },
    "matern_s05_n0_advec_zero": {
        "family": "matern",
        "smooth": 0.5,
        "fixed_nugget": 0.0,
        "fixed_advec_lat": 0.0,
        ...
        "gc_alpha": np.nan,
        "gc_beta": np.nan,
        "label": "Matern s=0.5 nugget0 advec zero",
        "color": "#1f77b4",
    },
    "matern_s05_n0_advec_large": {
        "family": "matern",
        "smooth": 0.5,
        "fixed_nugget": 0.0,
        "fixed_advec_lat": 0.5,
        ...
        "gc_alpha": np.nan,
        "gc_beta": np.nan,
        "label": "Matern s=0.5 nugget0 advec 0.5/0.5",
        "color": "#d62728",
    },
...
DEFAULT_BOTH_EXPERIMENTS = ["advection_mismatch"]
DGP1_BOTH_EXPERIMENTS = ["advection_mismatch_dgp1"]
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Smooth=0.5 one-day ST Vecchia conditional eigen advection-mismatch simulation.")
    parser.add_argument("--experiment", choices=[*sorted(EXPERIMENT_VARIANTS), "both", "both_dgp1"], default="advection_mismatch_dgp1")
...
def main() -> None:
    ...
    print("\nAll requested advection-mismatch experiments completed.", flush=True)
