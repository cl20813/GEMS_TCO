#!/usr/bin/env python3
"""2022 real-data ST corridor Vecchia smooth/prefix validation.

This run narrows the 2022 ambiguity by fitting fixed smooth values
0.25, 0.30, and 0.35 over increasing 4x4 block-center max-min prefixes:
100, 200, 400, 600, 800, and all blocks.

It writes monthly parameter trend plots plus likelihood/objective heatmaps so
smooth candidates can be compared within each prefix.
"""
...
if str(SPACE_TIME_DIR) not in sys.path:
    sys.path.insert(0, str(SPACE_TIME_DIR))
...
ROUND_DECIMALS = 6
ALL_FITS_CSV = "real_july2022_corridor_lag643_smooth_prefix_all_fits.csv"
SUMMARY_CSV = "real_july2022_corridor_lag643_smooth_prefix_monthly_summary.csv"
LIKELIHOOD_CSV = "real_july2022_corridor_lag643_smooth_prefix_likelihood_table.csv"
MISSING_PREFIX_CSV = "real_july2022_corridor_lag643_smooth_prefix_missing.csv"


def default_output_root() -> Path:
    if Path("/home/jl2815").exists():
        return Path("/home/jl2815/tco/exercise_output/summer/real_july2022_corridor_lag643_smooth_prefix_060626")
    return Path("/Users/joonwonlee/Documents/GEMS_TCO-1/outputs/summer_26/real_july2022_corridor_lag643_smooth_prefix_060626")
...
def blockmaxmin_indices(grid_coords_np: np.ndarray, n_blocks_requested: int, block_shape: tuple[int, int] = BLOCK_SHAPE) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    n_use = int(len(block_keys)) if requested <= 0 else min(requested, int(len(block_keys)))
...
def plot_dataset_monthly(summary: pd.DataFrame, dataset: str, path: Path) -> None:
    ...
    if not x_order:
        ...
        return
    label_map = (
        sub_dataset[["block_prefix_requested", "block_prefix_label"]]
        .drop_duplicates()
        .set_index("block_prefix_requested")["block_prefix_label"]
        .to_dict()
    )
    x_labels = [str(label_map.get(x, x)) for x in x_order]
...
def make_likelihood_summary(ok: pd.DataFrame) -> pd.DataFrame:
    if ok.empty:
        return pd.DataFrame()
    group_cols = [
        "dataset",
    ...
    for keys, sub in ok.groupby(group_cols, dropna=False):
        ...
        rows.append(
            {
                "dataset": str(keys[0]),
                "fit_smooth": float(keys[1]),
                "year": int(keys[2]),
                "month": int(keys[3]),
                "block_prefix_requested": int(keys[4]),
                "block_prefix_used": int(keys[5]),
                "block_prefix_label": str(keys[6]),
                "block_prefix_order": int(keys[7]),
...
def plot_likelihood_heatmaps(like: pd.DataFrame, out_dir: Path, monthly_out_dir: Path | None = None) -> None:
    if like.empty:
        return
    ...
    for metric, title in [
        ("loss_per_valid_median", "median objective per valid observation"),
        ("loss_median", "median final objective"),
    ]:
        table = like.pivot_table(
            index="fit_smooth",
        ...
        if table.empty:
            continue
        col_order = (
            like[["block_prefix_label", "block_prefix_order"]]
            .drop_duplicates()
        ...
        fig, ax = plt.subplots(figsize=(1.35 * max(4, table.shape[1]), 1.0 * max(3, table.shape[0]) + 1.6))
        arr = table.to_numpy(dtype=float)
        im = ax.imshow(arr, aspect="auto", cmap="viridis")
        ax.set_xticks(np.arange(table.shape[1]))
        ax.set_xticklabels(table.columns.astype(str), rotation=0)
        ax.set_yticks(np.arange(table.shape[0]))
        ax.set_yticklabels([f"{x:g}" for x in table.index.to_numpy(dtype=float)])
        ax.set_xlabel("4x4 block-center max-min prefix")
        ax.set_ylabel("fixed smooth")
        ax.set_title(f"2022 {title}")
        ...
        for plot_dir in plot_dirs:
            fig.savefig(plot_dir / f"real_2022_likelihood_heatmap_{safe_metric}.png", dpi=180, bbox_inches="tight")
        plt.close(fig)
...
def refresh_outputs(
    ...
    like = make_likelihood_summary(ok) if not ok.empty else pd.DataFrame()
    if not like.empty:
        save_rows(out_dir / LIKELIHOOD_CSV, like.to_dict(orient="records"))
        plot_likelihood_heatmaps(like, out_dir, monthly_out_dir)
    lines = [
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="2022 real-data ST corridor Vecchia smooth/prefix validation.")
    parser.add_argument("--datasets", nargs="+", default=["real"], choices=["real", "sim0p3", "sim0p5"])
    parser.add_argument("--real-fit-smooths", nargs="+", default=["0.25", "0.3", "0.35"])
    parser.add_argument(
        "--real-fit-smooths-by-year",
        nargs="*",
        default=["2022:0.25,0.3,0.35"],
    ...
    parser.add_argument("--real-years", nargs="+", default=["2022"])
