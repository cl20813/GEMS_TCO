...
scp \
  ...
  "${LOCAL_DIR}/vecchia_real10_selected5x2_adapted_fixed_nll_lanczos_eigen512_fulleigen_logic_lag643.py" \
  "${LOCAL_DIR}/slurm_vecchia_real10_selected5x2_adapted_fixed_nll_lanczos_eigen512_fulleigen_logic_lag643.sh" \
  "${PRECISION_HELPER}" \
  "${REMOTE_HOST}:${REMOTE_DIR}/"

echo "Upload complete. Submit with:"
echo "ssh ${REMOTE_HOST} 'cd ${REMOTE_DIR} && sbatch slurm_vecchia_real10_selected5x2_adapted_fixed_nll_lanczos_eigen512_fulleigen_logic_lag643.sh'"
echo
echo "Monitor with:"
echo "ssh ${REMOTE_HOST} 'squeue -u jl2815'"
echo
