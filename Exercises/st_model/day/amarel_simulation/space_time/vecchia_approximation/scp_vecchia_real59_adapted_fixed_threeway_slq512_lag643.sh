...
scp \
  ...
  "${LOCAL_DIR}/slurm_vecchia_real59_adapted_fixed_threeway_slq512_lag643.sh" \
  "${PRECISION_HELPER}" \
  "${REMOTE_HOST}:${REMOTE_DIR}/"

echo "Upload complete. Submit with:"
echo "ssh ${REMOTE_HOST} 'cd ${REMOTE_DIR} && sbatch slurm_vecchia_real59_adapted_fixed_threeway_slq512_lag643.sh'"
echo
echo "After completion, download with:"
echo "scp -r ${REMOTE_HOST}:/home/jl2815/tco/exercise_output/summer/vecchia_real59_adapted_fixed_threeway_slq512_lag643 '${LOCAL_DIR}/'"
