...
def cli(
    ...
    for day in range(days):
        ...
        out, epoch = model_instance.run_vecc_reorder(params, optimizer,scheduler, model_instance.matern_cov_anisotropy_v05, cov_map, epochs=epochs)
        end_time = time.time()
        epoch_time = end_time - start_time
        print(f"2024-07-{day+1}", "vecchia_v05_reord", (200 / lat_lon_resolution[0]) * (100 / lat_lon_resolution[0]) , lr,  step , out, epoch_time, epoch)
        
        input_filepath = input_path / f"vecchia_v05_reord_{ (200 / lat_lon_resolution[0]) * (100 / lat_lon_resolution[0]) }.json"
        ...
        csv_filepath = input_path/f"vecchia_v05_reord_{(200 / lat_lon_resolution[0]) * (100 / lat_lon_resolution[0])}.csv"
