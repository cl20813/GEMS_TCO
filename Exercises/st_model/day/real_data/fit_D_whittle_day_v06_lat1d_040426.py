"""
fit_D_whittle_day_v06_lat1d_040426.py

Debiased Whittle fitting — lat-only 1D differencing version.

Change from v05 ([-2,1;1,0]):
  - Uses debiased_whittle_lat1d instead of debiased_whittle
  - Differencing: Z(i,j) = X(i+1,j) - X(i,j)  (lat direction only)
  - Output grid: (nlat-1) x nlon  (vs. (nlat-1) x (nlon-1) in v05)
  - Covariance: Cov_Z = 2*C_X(u) - C_X(u-d1) - C_X(u+d1)  (lat shift only)

Motivation:
  Spatial non-stationarity is a lat-direction gradient.
  Differencing in lon direction was unnecessary and removed valid signal.
"""
...
def cli(
    ...
    days: List[str] = typer.Option(['0', '31'], help="Start and End day index (0-based)"),
    ...
    output_path = Path(config.amarel_estimates_day_path) / "july_22_23_24_25_lat1d"
    ...
    for year in years_list:
        ...
        for day_idx in days_list:
            print(f'\n{"="*50}')
            print(f'--- DW-lat1d: Day {day_idx+1} ({year}-{month:02d}-{day_idx+1}) ---')
            print(f'{"="*50}')
            ...
            try:
                ...
                del obs_masks
                print(f"Grid: {n1}x{n2} (lat-only diff → nlon unchanged), {p_time} time pts.")
                ...
                common_filename = f"real_dw_lat1d_july_22_23_24_25"
