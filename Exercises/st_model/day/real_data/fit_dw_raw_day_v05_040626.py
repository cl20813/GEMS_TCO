"""
fit_dw_raw_day_v05_040626.py

Debiased Whittle fitting — RAW baseline (no spatial differencing).

  Z(i,j,t) = X(i,j,t)   (identity filter, H(ω) = 1)
  Cov_Z(u,v,τ) = C_X(u·δ_lat, v·δ_lon, τ)  directly — no filter cross-terms.
  Output grid : nlat × nlon  (no size reduction).
  DC handling : per-slice spatial demean + ω=(0,0) excluded from likelihood.

Module: debiased_whittle_raw

Compare with:
  fit_dw_2conv_day_v05_040626.py  — full 2D-diff DW
  fit_dw_mix_day_v05_040626.py    — mixed-frequency DW

Baseline purpose:
  Performance ceiling for parameter estimation when data is (near-)stationary.
  Gridification bias reference without any filter distortion.

Usage:
  python fit_dw_raw_day_v05_040626.py --days "0,28" --years "2022,2023,2024,2025"
"""
...
def cli(
    ...
    print(f"Output grid     : nlat × nlon  (no size reduction)")
    print(f"DC handling     : per-slice demean + ω=(0,0) excluded")
    print(f"Target Days     : {days_list}")
    print(f"Target Years    : {years_list}")

    month_range = [month]

    output_path = Path(config.amarel_estimates_day_path) / "july_22_23_24_25_raw"
    ...
    for year in years_list:
        ...
        for day_idx in days_list:
            print(f'\n{"="*50}')
            print(f'--- DW-raw: Day {day_idx+1} ({year}-{month:02d}-{day_idx+1}) ---')
            print(f'{"="*50}')
            ...
            try:
                ...
                del obs_masks
                print(f"Grid: {n1}×{n2} (raw — full grid, no reduction), {p_time} time pts.")
                ...
                print(f"Raw DW finished in {epoch_time:.2f}s.")
                ...
                common_filename = f"real_dw_raw_july_22_23_24_25"
