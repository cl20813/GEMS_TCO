...
def cli(
    ...
    for day in range(days):
        ...
        out, epoch = model_instance.run_full(params, optimizer,scheduler, model_instance.matern_cov_anisotropy_kv, epochs=epochs)
    
        end_time = time.time()
        epoch_time = end_time - start_time
        print(f"2025-07-{day+1}", "full_v10", (200 / lat_lon_resolution[0]) * (100 / lat_lon_resolution[0]) , lr,  step , out, epoch_time, epoch)

        input_path = Path("/home/jl2815/tco/exercise_output/estimates/day/")
        input_filepath = input_path / f"full_v10_{ (200 / lat_lon_resolution[0]) * (100 / lat_lon_resolution[0]) }.json"
        res = alg_optimization( f"2025-07-{day+1}", "Vecc_b2", (200 / lat_lon_resolution[0]) * (100 / lat_lon_resolution[0]) , lr,  step , out, epoch_time, epoch)
        ...
        csv_filepath = input_path/f"full_v10_{(200 / lat_lon_resolution[0]) * (100 / lat_lon_resolution[0])}.csv"
