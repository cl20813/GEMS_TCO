...
def cli(
    ...
    def calculate_original_scale_metrics(est_params, true_init_dict):
        ...
        # (Est - True) / True 를 통해 모든 변수를 '백분율 오차' 스케일로 통일
        # 분모가 0이 되는 것을 방지하기 위해 아주 작은 값(eps)을 더할 수 있으나, 
        # 오존 모델 파라미터 특성상 0이 없다면 그대로 진행합니다.
        relative_error = (est_array - true_array) / true_array

        # 4. Calculate RMSRE (Root Mean Square Relative Error)
        msre = torch.mean(relative_error ** 2)
        rmsre = torch.sqrt(msre).item()
...
# -------------------------------------------------------
        ...
        input_filepath = output_path / f"sim_irre_dw_{a_date}_{ ( daily_aggregated_tensors_dw[0].shape[0]/8 ) }.json"
        ...
        csv_filepath = input_path/f"sim_irre_dW_v{int(v*100):03d}_{a_date}_{(daily_aggregated_tensors_vecc[0].shape[0]/8 )}.csv"
        ...
        input_filepath = output_path / f"sim_irre_vecc_{a_date}_{ ( daily_aggregated_tensors_vecc[0].shape[0]/8 ) }.json"
        ...
        csv_filepath = input_path/f"sim_irre_vecc_{a_date}_v{int(v*100):03d}_LBFGS_{(daily_aggregated_tensors_vecc[0].shape[0]/8 )}.csv"
        res.tocsv( loaded_data, fieldnames,csv_filepath )
