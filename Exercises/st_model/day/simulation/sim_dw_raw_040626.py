...
Comparison targets:
  - sim_three_model_comparison_031926.py  → DW_2d  (2D separable filter [-1,1;1,-1])
  - sim_dw_lat1d_040526.py                → DW_lat1d (1D lat-only filter [-1;1])

Design:
  - Same data generation pipeline (FFT circulant + step3 obs→cell)
  - Same true parameters (Scenario D)
  - Same random init scheme
  - Only change: NO spatial filter applied before the Whittle likelihood.
    Each time slice is spatially demeaned (subtract mean of observed cells)
    to satisfy the zero-mean GP assumption; the DC frequency ω=(0,0) is
    additionally excluded from the likelihood sum as a belt-and-suspenders
    safeguard against residual DC contamination.
...
def assemble_reg_dataset(field, step3_per_t, hr_idx_per_t, src_locs_per_t,
    ...
    reg_map[key]: [N_grid, 11]
      col 0,1: grid lat/lon
      col 2  : sim_val (NaN for unassigned cells)
      col 3  : time slot index
      col 4+ : dummy day indicators
    """
...
def cli(
    ...
    print(f"  After raw passthrough: {n_lat} lat × {n_lon} lon = {N_grid} cells "
    ...
    for it in range(num_iters):
        ...
        try:
            ...
            # ── Raw DW preprocess ─────────────────────────────────────────────
            # debiased_whittle_preprocess.apply_raw_passthrough:
            #   (1) validates grid completeness
            #   (2) subtracts spatial mean of observed cells (demean per time slice)
            #   (3) returns full nlat × nlon grid
            ...
            # generate_spatially_filtered_days: subset + demean, NO spatial filter
            # output grid stays nlat × nlon
            ...
            print(f"  DW grid: {n1}×{n2}, {p_time} time steps  "
                  f"(raw: full grid, no lat/lon reduction)")
        ...
        for lbl, col, tv in zip(P_LABELS, P_COLS, TRUE_VALS):
            ...
            print(f"  {lbl:<12} {tv:>{cw}.4f}  {mean_:>{cw}.4f}  {bias_:>{cw}.4f}  "
                  f"{rmsre_:>{cw}.4f}  {mdare_:>{cw}.4f}  {p90p10_:>{cw}.4f}")
        rmsre_all = np.array([r['rmsre'] for r in records])
        print(f"  {'-'*80}")
        print(f"  {'Overall':<12} {'':>{cw}}  {'':>{cw}}  {'':>{cw}}  "
              f"{np.mean(rmsre_all):>{cw}.4f}  {np.median(rmsre_all):>{cw}.4f}  "
              f"{np.percentile(rmsre_all,90)-np.percentile(rmsre_all,10):>{cw}.4f}")
    ...
    summary_rows.append({
        'param':  'Overall', 'true': float('nan'), 'mean': float('nan'),
    ...
    print(f"\nFinal summary:\n"
          f"{df_summary[['param','true','mean','bias','RMSRE','MdARE']].to_string(index=False)}")
