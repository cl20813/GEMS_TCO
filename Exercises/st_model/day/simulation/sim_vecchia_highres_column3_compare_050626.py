"""
sim_vecchia_highres_column3_compare_050626.py

Amarel high-resolution complete-grid simulation comparing:
  1. Original regular-grid Reverse-L Column, nominal m=252
  2. Column V2 rigid reverse-L/downward-right, nominal m=150
  3. Hybrid Lean batched Vecchia, nominal m=41
...
from GEMS_TCO.vecchia_candidate.kernels_vecchia_grid_col_template_reuse import ReverseLColumnVecchiaFit
...
def fit_original_column(reg_map_ord, ordered_grid_coords_np, initial_vals, truth, args):
    ...
    model = ReverseLColumnVecchiaFit(
        ...
        head_right_cols=ORIG_COLUMN_SPEC["head_right_cols"],
        above_count=ORIG_COLUMN_SPEC["above_count"],
        right_col_count=ORIG_COLUMN_SPEC["right_col_count"],
        right_neighbor_count=ORIG_COLUMN_SPEC["right_neighbor_count"],
        lag_count=ORIG_COLUMN_SPEC["lag_count"],
        include_lag_self=ORIG_COLUMN_SPEC["include_lag_self"],
    ...
    return finish_row(
        ...
        metrics,
        {
            "total_conditioning_nominal": (ORIG_COLUMN_SPEC["above_count"] + ORIG_COLUMN_SPEC["right_neighbor_count"])
            * (ORIG_COLUMN_SPEC["lag_count"] + 1),
...
def fit_column_v2(reg_map_ord, ordered_grid_coords_np, initial_vals, truth, args):
    params = [torch.tensor([v], device=DEVICE, dtype=DTYPE, requires_grad=True) for v in initial_vals]
    model = ReverseLColumnVecchiaFitV2(
        smooth=SMOOTH,
        input_map=reg_map_ord,
        mm_cond_number=args.mm_cond_number,
        grid_coords=ordered_grid_coords_np,
...
def main():
    ...
    parser.add_argument("--out-prefix", type=str, default="sim_vecchia_highres_column3_compare_050626")
    ...
    fitters = [
        ("original_column", fit_original_column),
        ("column_v2", fit_column_v2),
