...
Two-model DW simulation: gridification bias test.
  ...
  DW_2d_loc : FFT high-res → real GEMS obs locations → step3 (obs→cell, 1:1)
              ...
              (same pipeline as sim_dw2110_three_model_041626.py DW column)
...
Both models use debiased_whittle_1111 (2D separable filter [-1,1;1,-1]).
...
Usage:
  ...
  python sim_dw1111_041626.py --num-iters 1 --lat-factor 10 --lon-factor 4
  python sim_dw1111_041626.py --num-iters 300
"""
...
from GEMS_TCO import debiased_whittle_1111 as debiased_whittle
...
def apply_step3_1to1(src_np_valid, grid_coords_np, grid_tree):
    ...
    if filled.any():
        ...
        asgn[np.where(filled)[0][too_far]] = -1
    return asgn
