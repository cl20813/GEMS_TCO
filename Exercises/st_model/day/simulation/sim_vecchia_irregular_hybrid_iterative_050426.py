"""
sim_vecchia_irregular_hybrid_iterative_050426.py

Simulation study: Hybrid_Lean (local 8 + fresh 4 / local 4 + fresh 3, total=41)
with iterative lon-offset calibration.

Idea: begin with scientific prior offset (0.063 °/step, east-to-west wind region),
fit model, extract advec_lon estimate, use |advec_lon_est| as new lag-1 offset
(lag-2 is automatically 2× via multiplier=2.0), warm-start next iteration.
Repeat until |Δoffset| < tol or max_outer_iters reached.

Models compared:
  1. Irr_Baseline_A20_B18_C15        — local-only, no shift (reference)
  2. Hybrid_Lean_Fixed_0p063          — fixed offset 0.063
  3. Hybrid_Lean_Fixed_0p126          — fixed offset 0.126 (2× prior, oracle-ish)
  4. Hybrid_Lean_Iterative            — iterative offset (starts 0.063, self-calibrates)

SLURM array: --true-advec-lon -0.10,-0.16,-0.25

Example:
  conda activate faiss_env
  python sim_vecchia_irregular_hybrid_iterative_050426.py --num-iters 2 --true-advec-lon -0.16
"""
...
# ---------------------------------------------------------------------------
# Utility helpers (same as hybrid_compare)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Hybrid fresh class (unchanged from hybrid_compare)
# ---------------------------------------------------------------------------

class fit_vecchia_lbfgs_fresh_hybrid(kernels_vecchia.fit_vecchia_lbfgs):
    """Vecchia kernel: local lag neighbors + fresh shifted-center lag neighbors."""
...
def make_model_specs() -> List[Dict[str, Any]]:
    return [
        ...
        {
            "model":               "Irr_Baseline_A20_B18_C15",
        ...
        # ---- Hybrid Lean, fixed offset 0.063 --------------------------------
        {
            "model":               "Hybrid_Lean_Fixed_0p063",
        ...
        # ---- Hybrid Lean, fixed offset 0.126 --------------------------------
        {
            "model":               "Hybrid_Lean_Fixed_0p126",
            "group":               "hybrid_lean_fixed",
            "kernel":              "hybrid_fresh",
            "limit_A":             20, "limit_B": 8, "limit_C": 4,
            "lag1_local_count":    8,  "lag1_fresh_count": 4,
        ...
        # ---- Hybrid Lean, iterative offset (key new model) ------------------
        {
            "model":               "Hybrid_Lean_Iterative",
            "group":               "hybrid_lean_iterative",
            "kernel":              "hybrid_fresh",
            "limit_A":             20, "limit_B": 8, "limit_C": 4,
            "lag1_local_count":    8,  "lag1_fresh_count": 4,
            "lag2_local_count":    4,  "lag2_fresh_count": 3,
            "pred_lag1_lon_offset": 0.063,   # initial prior offset
            "total_conditioning":  41,
            "iterative":           True,
        },
    ]
...
# ---------------------------------------------------------------------------
# Single-pass fitting (non-iterative models)
# ---------------------------------------------------------------------------
...
def fit_irregular_model(spec, irr_map_ord, nns_grid, ordered_grid_coords_np,
    ...
    return (out, loss, int(fit_iter) + 1, precompute_s, fit_s, gim_s,
            metrics, est, godambe, [spec["pred_lag1_lon_offset"]], 1)


# ---------------------------------------------------------------------------
# Iterative fitting (self-calibrating offset)
# ---------------------------------------------------------------------------

def fit_iterative_hybrid_model(
    spec, irr_map_ord, nns_grid, ordered_grid_coords_np,
    initial_vals, smooth, mm_cond_number, nheads, daily_stride,
    lbfgs_lr, lbfgs_eval, lbfgs_hist, lbfgs_steps,
    max_outer_iters=4, offset_tol=0.01, min_offset=0.02,
    compute_godambe=True, true_dict=None,
):
    """Iteratively update lag-1 lon offset from fitted advec_lon.

    Iteration k:
      offset_k = |advec_lon estimated at iteration k-1|
      lag-2 offset is automatically 2 × offset_k (via multiplier=2.0 in BallTree lookup)

    Warm-start: params from previous iteration initialize next LBFGS.
    Godambe is computed only on the final iteration.

    Returns same signature as fit_irregular_model plus:
      offset_history  — list of offsets tried
      n_outer_iters   — number of outer iterations actually performed
    """
    ...
    for outer_it in range(max_outer_iters):
        is_final = (outer_it == max_outer_iters - 1)
        ...
        model = fit_vecchia_lbfgs_fresh_hybrid(
            ...
            daily_stride=daily_stride,
            spatial_coords=ordered_grid_coords_np,
            lag1_lon_offset=current_offset,
            lag1_fresh_count=spec["lag1_fresh_count"],
            lag2_fresh_count=spec["lag2_fresh_count"],
        )
        model.precompute_conditioning_sets()
        total_precompute_s += time.time() - t_pre

        params = [torch.tensor([val], device=DEVICE, dtype=DTYPE, requires_grad=True)
                  for val in current_init]
        optimizer = model.set_optimizer(params, lr=lbfgs_lr, max_iter=lbfgs_eval,
                                         history_size=lbfgs_hist)
        t_fit = time.time()
        # Use half the LBFGS budget for early (exploratory) outer iterations,
        # full budget only for the final iteration.
        steps_this = lbfgs_steps if is_final else max(2, lbfgs_steps // 2)
        out, fit_iter = model.fit_vecc_lbfgs(params, optimizer, max_steps=steps_this, grad_tol=1e-5)
        total_fit_s += time.time() - t_fit

        est = backmap_params(out)
        new_offset = max(float(abs(est["advec_lon"])), min_offset)

        print(
            f"  [outer={outer_it}] offset={current_offset:.4f}  "
            f"advec_lon_est={est['advec_lon']:.4f}  "
            f"→ new_offset={new_offset:.4f}"
        )
        ...
        if delta < offset_tol:
            print(f"  Converged (|Δoffset|={delta:.4f} < {offset_tol})")
            break

    n_outer = len(offset_history) - 1   # number of fitting iterations done

    # If we converged early, do one final full-budget fit with converged offset
    # (only needed when the convergence happened before the last outer_it)
    if n_outer < max_outer_iters and not is_final:
        print(f"  Final full-budget fit with converged offset={current_offset:.4f}")
        t_pre = time.time()
        model = fit_vecchia_lbfgs_fresh_hybrid(
            smooth=smooth, input_map=irr_map_ord, nns_map=nns_grid,
            mm_cond_number=mm_cond_number, nheads=nheads,
    ...
    return (out, loss, int(fit_iter) + 1, total_precompute_s, total_fit_s, gim_s,
            metrics, est, godambe, offset_history, n_outer)
...
def build_model_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    group_cols = ["model", "group", "kernel", "pred_lag1_lon_offset_init",
                  "lag1_local_count", "lag1_fresh_count",
                  "lag2_local_count", "lag2_fresh_count", "total_conditioning",
                  "true_advec_lon", "iterative"]
    rows = []
    for keys, sub in ok.groupby(group_cols, dropna=False):
        row = dict(zip(group_cols, keys))
        row["n"] = len(sub)
        for col in METRIC_COLS + GIM_METRIC_COLS + ["n_outer_iters", "final_offset"]:
...
def build_offset_trajectory(df: pd.DataFrame) -> pd.DataFrame:
    """Summarize how iterative offsets evolved across simulation runs."""
    ok = df[(df["error"].fillna("") == "") & (df["iterative"] == True)].copy()
    if ok.empty:
        return pd.DataFrame()
    rows = []
    for _, row in ok.iterrows():
        hist_str = row.get("offset_history_str", "")
        if not hist_str:
            continue
        ...
        for step_i, off in enumerate(hist):
            rows.append({
                "iter": row["iter"],
                "true_advec_lon": row["true_advec_lon"],
                "outer_step": step_i,
                "offset": off,
                "n_outer_iters": row["n_outer_iters"],
...
def cli(
    ...
    true_advec_lon:  str   = typer.Option("-0.10,-0.16,-0.25",
                                           help="Comma-separated true lon advection values (cycled)"),
    ...
    max_outer_iters: int   = typer.Option(4,     help="Max offset calibration iterations (iterative model)"),
    offset_tol:      float = typer.Option(0.01,  help="Convergence threshold for offset update"),
    min_offset:      float = typer.Option(0.02,  help="Minimum allowed offset (prevents collapse to 0)"),
    compute_godambe: bool  = typer.Option(True,  help="Compute Godambe SE each iteration"),
    seed:            int   = typer.Option(42,    help="Random seed"),
    out_prefix:      str   = typer.Option("sim_vecchia_irregular_hybrid_iterative",
    ...
    print(f"True advec_lon list : {advec_lon_list}")
    print(f"Max outer iters     : {max_outer_iters}  tol={offset_tol}  min_offset={min_offset}")
    ...
    offset_traj_csv    = output_path / f"{out_prefix}_{date_tag}_offset_trajectory.csv"
    ...
    for s in model_specs:
        iterative_tag = " [ITERATIVE]" if s["iterative"] else ""
        print(
            f"  {s['model']}: kernel={s['kernel']}  "
            f"A={s['limit_A']}  B_local={s['lag1_local_count']}  B_fresh={s['lag1_fresh_count']}  "
            f"C_local={s['lag2_local_count']}  C_fresh={s['lag2_fresh_count']}  "
            f"init_offset={s['pred_lag1_lon_offset']:.3f}  total={s['total_conditioning']}"
            f"{iterative_tag}"
        )
    ...
    for it in range(num_iters):
        ...
        for spec in model_specs:
            ...
            try:
                if spec["iterative"]:
                    (out, loss, fit_iter, pre_s, fit_s, gim_s,
                     metrics, est, godambe, offset_history, n_outer) = fit_iterative_hybrid_model(
                    ...
                    (out, loss, fit_iter, pre_s, fit_s, gim_s,
                     metrics, est, godambe, offset_history, n_outer) = fit_irregular_model(
                        spec, irr_map_ord, nns_grid, ordered_grid_coords_np, initial_vals,
                        v, mm_cond_number, nheads, daily_stride,
                        lbfgs_lr, lbfgs_eval, lbfgs_hist, lbfgs_steps,
                        compute_godambe=compute_godambe,
                ...
                print(
                    ...
                    f"advec_lon_est={adv_lon_est:.4f}  final_offset={final_offset:.4f}  "
                    f"n_outer={n_outer}  "
                    f"gim_relse_advec={gim_adv_relse:.4f}  time={total_s:.1f}s"
                )
            except Exception as exc:
                total_s      = time.time() - pre_t
                loss = pre_s = fit_s = gim_s = fit_iter = float("nan")
                metrics      = {k: float("nan") for k in
                                ["overall_rmsre", "spatial_rmsre", "range_time_re", "advec_rmsre",
                                 "nugget_re", "sigmasq_re", "range_lat_re", "range_lon_re",
                                 "advec_lat_re", "advec_lon_re"]}
                est          = {k: float("nan") for k in P_LABELS}
                godambe      = {}
                offset_history = [spec["pred_lag1_lon_offset"]]
                n_outer      = 0
                error_msg    = f"{type(exc).__name__}: {exc}"
                print(f"FAILED: {error_msg}")
                final_offset = spec["pred_lag1_lon_offset"]
            ...
            records.append({
                ...
                "offset_history_str":          "|".join(f"{o:.6f}" for o in offset_history),
        ...
        if not ok_now.empty:
            ...
            otraj   = build_offset_trajectory(df_now)
            ...
            otraj.to_csv(offset_traj_csv,    index=False)

            if not msumm.empty:
                print("\nRunning model summary")
                show_cols = [
                    "model", "true_advec_lon", "pred_lag1_lon_offset_init",
                    "final_offset_mean", "n_outer_iters_mean",
    ...
    otraj   = build_offset_trajectory(df_final)
    msumm.to_csv(model_summary_csv,  index=False)
    psumm.to_csv(param_summary_csv,  index=False)
    gimsumm.to_csv(gim_summary_csv,  index=False)
    otraj.to_csv(offset_traj_csv,    index=False)
    ...
    print(f"Saved offset trajectory: {offset_traj_csv}")
