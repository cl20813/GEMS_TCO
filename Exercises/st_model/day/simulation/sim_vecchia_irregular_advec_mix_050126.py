"""
sim_vecchia_irregular_advec_mix_050126.py

Amarel simulation study for advection-aware reduced lag conditioning on
real-data-like irregular GEMS observation patterns.

Compares standard and advection-mixed Vecchia-Irregular models on exactly the
same simulated data:

  1. Irr_Full_A20_B20_C20
     Original 20/20/20 irregular Vecchia budget.

  2. Irr_Cand_A20_B18_C15
     Strong reduced baseline that preserved recovery in the lag-ratio run.

  3. Irr_Ratio_A20_B16_C10
     Aggressive reduced baseline that was fast but unstable for advection.

  4+. Advection-mixed candidates:
      keep A=20 and reduce lag budgets below the 20/18/15 standard candidate,
      but reserve some lag slots for fixed upstream longitude-band points. For
      negative longitudinal advection, upstream at previous times is east of
      the target:
        t-1 offsets: +2,+3 or +1,+2,+3,+4 longitude cells
        t-2 offsets: +4,+5 or +3,+4,+5,+6 longitude cells

The data pipeline follows existing real-data-like high-resolution simulation pipeline:
...
Example:
  ...
  python sim_vecchia_irregular_advec_mix_050126.py --num-iters 1
"""
...
def build_model_summary(df):
    ...
    group_cols = [
        ...
        "lag1_advec_offsets", "lag2_advec_offsets", "total_conditioning",
    ]
...
def cli(
    ...
    out_prefix: str = typer.Option("sim_vecchia_irregular_advec_mix", help="Output prefix"),
) -> None:
    set_seed(seed)
    ...
    model_specs = [
        ...
        },
        {
            ...
            "allocation": "best standard reduced: t-1 0.9x, t-2 0.75x",
            "kernel": "std",
            "limit_A": 20,
            "limit_B": 18,
            "limit_C": 15,
        },
        {
            ...
            "allocation": "aggressive standard reduced: t-1 near 0.8x, t-2 near 0.5x",
            "kernel": "std",
        ...
        {
            "model": "Irr_Std_A20_B14_C08",
            "allocation": "smaller standard control matching compact advec budgets",
            "kernel": "std",
            "limit_A": 20,
            "limit_B": 14,
            "limit_C": 8,
        ...
        {
            "model": "Irr_Std_A20_B12_C06",
            "allocation": "very small standard control for advec-rescue comparison",
            "kernel": "std",
            "limit_A": 20,
            "limit_B": 12,
            "limit_C": 6,
        ...
        {
            "model": "Irr_AdvMix_A20_B14p2_C08p2",
            "allocation": "same total as 20/16/10: local B14+C8 plus upstream B2+C2",
            "kernel": "advec_mixed",
            "limit_A": 20,
            "limit_B": 14,
            "limit_C": 8,
            "lag1_advec_cell_offsets": (2, 3),
            "lag2_advec_cell_offsets": (4, 5),
        },
        {
            "model": "Irr_AdvMix_A20_B14p2_C06p4",
            "allocation": "same total as 20/16/10: local B14+C6 plus upstream B2+C4",
            "kernel": "advec_mixed",
            "limit_A": 20,
            "limit_B": 14,
            "limit_C": 6,
            "lag1_advec_cell_offsets": (2, 3),
            "lag2_advec_cell_offsets": (3, 4, 5, 6),
        },
        {
            "model": "Irr_AdvMix_A20_B12p4_C06p4",
            "allocation": "same total as 20/16/10: wider upstream bands, fewer local lag neighbors",
            "kernel": "advec_mixed",
            "limit_A": 20,
            "limit_B": 12,
            "limit_C": 6,
            "lag1_advec_cell_offsets": (1, 2, 3, 4),
            "lag2_advec_cell_offsets": (3, 4, 5, 6),
        },
        {
            "model": "Irr_AdvMix_A20_B12p2_C08p2",
            "allocation": "total 46: compact local lags plus narrow upstream bands",
            "kernel": "advec_mixed",
            "limit_A": 20,
            "limit_B": 12,
            "limit_C": 8,
        ...
        {
            "model": "Irr_AdvMix_A20_B12p2_C06p4",
            "allocation": "total 46: compact local lags plus wider t-2 upstream band",
            "kernel": "advec_mixed",
            "limit_A": 20,
            "limit_B": 12,
            "limit_C": 6,
        ...
        {
            "model": "Irr_AdvMix_A20_B10p4_C06p4",
            "allocation": "total 46: advec-heavy compact budget",
            "kernel": "advec_mixed",
            "limit_A": 20,
            "limit_B": 10,
            "limit_C": 6,
        ...
        {
            "model": "Irr_AdvMix_A20_B10p2_C06p4",
            "allocation": "total 44: most aggressive advec-rescue candidate",
            "kernel": "advec_mixed",
            "limit_A": 20,
            "limit_B": 10,
            "limit_C": 6,
        ...
        {
            "model": "Irr_AdvMix_A20_B16p2_C10p2",
            "allocation": "total 52: local 20/16/10 plus narrow upstream bands",
            "kernel": "advec_mixed",
            "limit_A": 20,
            "limit_B": 16,
            "limit_C": 10,
        ...
        {
            "model": "Irr_AdvMix_A20_B16p2_C08p4",
            "allocation": "small overhead: local B16+C8 plus upstream B2+C4",
            "kernel": "advec_mixed",
            "limit_A": 20,
            "limit_B": 16,
            "limit_C": 8,
        ...
        {
            "model": "Irr_AdvMix_A20_B14p4_C08p4",
            "allocation": "total 52: wider upstream bands with moderate local lags",
            "kernel": "advec_mixed",
            "limit_A": 20,
            "limit_B": 14,
            "limit_C": 8,
        ...
        {
            "model": "Irr_AdvMix_A20_B16p4_C08p4",
            "allocation": "total 54: largest advec-mixed budget still below 20/18/15",
            "kernel": "advec_mixed",
            "limit_A": 20,
            "limit_B": 16,
            "limit_C": 8,
    ...
    for it in range(num_iters):
        ...
        if not ok_now.empty:
            ...
            if not model_summary.empty:
                ...
                cols = [
                    "model", "kernel", "n", "loss_mean", "overall_rmsre_mean",
