"""
sim_gim_vecc_cauchy_irr_dw_032826.py  —  Amarel (cluster) version

Computes GIM sandwich standard errors for DW, Vecchia-Irr, and Cauchy-Vecchia models
using **observed information** for J (no parametric bootstrap):
  ...
  - CY  J: per-unit conditional score outer products  (N_heads + N_tails terms)
...
Days are skipped if ANY of the three models (DW, VC, CY) is missing an estimate
for that day in the respective input CSVs.

Run via sbatch; see slurm_gim_vecc_cauchy_irr_dw_032826.md for the job script.

Output
------
[Mean GIM SE]: asymptotic standard errors from the Godambe sandwich estimator —
the primary output. DW SE vs VC SE vs CY SE with pairwise ratios per parameter.
...
_EST_COLS_CY  = [f"CY_Est_{p}"  for p in P_NAMES]
_SE_COLS_CY   = [f"CY_SE_{p}"   for p in P_NAMES]
...
def print_running_summary(records: list, n_total: int):
    ...
    # ── Mean GIM SE ──────────────────────────────────────────────────────────
    print(f"  {'param':<11} {'DW SE':>{cw}} {'VC SE':>{cw}} {'CY SE':>{cw}} {'DW/VC':>{cw}} {'DW/CY':>{cw}}")
    print(f"  {'-'*65}")
    for p, sc_dw, sc_vc, sc_cy in zip(P_NAMES, _SE_COLS_DW, _SE_COLS_VC, _SE_COLS_CY):
        dw_se = np.mean([r[sc_dw] for r in records])
        vc_se = np.mean([r[sc_vc] for r in records])
        cy_se = np.mean([r[sc_cy] for r in records])
        print(f"  {p:<11} {dw_se:>{cw}.4f} {vc_se:>{cw}.4f} {cy_se:>{cw}.4f} "
              f"{dw_se/(vc_se+1e-12):>{cw}.4f} {dw_se/(cy_se+1e-12):>{cw}.4f}")
...
def print_final_summary(records: list):
    ...
    print(f"  {'Parameter':<14} {'DW SE':>{cw}} {'VC SE':>{cw}} {'CY SE':>{cw}} {'DW/VC':>{cw}} {'DW/CY':>{cw}}")
    print(f"  {'-'*68}")
    for p, sc_dw, sc_vc, sc_cy in zip(P_NAMES, _SE_COLS_DW, _SE_COLS_VC, _SE_COLS_CY):
        dw_se = np.mean([r[sc_dw] for r in records])
        vc_se = np.mean([r[sc_vc] for r in records])
        cy_se = np.mean([r[sc_cy] for r in records])
        print(f"  {p:<14} {dw_se:>{cw}.4f} {vc_se:>{cw}.4f} {cy_se:>{cw}.4f} "
              f"{dw_se/(vc_se+1e-12):>{cw}.4f} {dw_se/(cy_se+1e-12):>{cw}.4f}")
...
def cli(
    ...
    print(f"Cauchy β={gc_beta}")
    print(f"Observed J — no bootstrap")

    output_path = Path(config.amarel_estimates_day_path) / "GIM"
    output_path.mkdir(parents=True, exist_ok=True)
    out_raw     = output_path / "GIM_cauchy_all_july_22_23_24_25_obsJ.csv"
    out_summary = output_path / "GIM_cauchy_summary_july_22_23_24_25_obsJ.csv"
    ...
    for label, path in [("DW", dw_csv), ("VC", vecc_csv), ("CY", cauchy_csv)]:
        if not path.exists():
            print(f"[Error] Missing {label} estimate CSV: {path}")
            raise SystemExit(1)
    ...
    cauchy_df = pd.read_csv(cauchy_csv)
    ...
    cauchy_by_day = {_norm_day(row['day']): [row[c] for c in _param_cols] for _, row in cauchy_df.iterrows()}
    print(f"Loaded estimate CSVs — {len(dw_by_day)} DW / {len(vecc_by_day)} VC / {len(cauchy_by_day)} CY days available")
    ...
    for year_str in years_list:
        ...
        for day in range(day_start, day_end + 1):
            ...
            # Skip if any model is missing estimates for this day
            missing = [m for m, d in [("DW", dw_by_day), ("VC", vecc_by_day), ("CY", cauchy_by_day)]
                       if day_str not in d]
            if missing:
                n_skip += 1
                print(f"  [SKIP] No estimates for {day_str} in: {', '.join(missing)}")
                continue

            print(f"  DW     params: {[round(x,4) for x in dw_by_day[day_str]]}")
            print(f"  Vecc   params: {[round(x,4) for x in vecc_by_day[day_str]]}")
            print(f"  Cauchy params: {[round(x,4) for x in cauchy_by_day[day_str]]}")
            ...
            try:
                ...
                beta_hat_vc = model_vc.get_gls_beta(vc_log_phi).detach()

                def per_unit_losses_vc(p):
                    return model_vc.vecchia_per_unit_nll_terms(p, beta_hat_vc)
                ...
                del model_vc, H_vc, J_vc, GIM_vc, J_mat_vc
                gc.collect(); torch.cuda.empty_cache()

                # ── [3/3] Cauchy GIM ──────────────────────────────────────────
                print(f"  [3/3] Cauchy GIM (β={gc_beta})...")
                cy_log_phi = torch.tensor(
                    transform_raw_to_log_phi(cauchy_by_day[day_str]),
                    device=DEVICE, dtype=DTYPE, requires_grad=True,
                )

                model_cy = kernels_vecchia_cauchy.fit_cauchy_vecchia_lbfgs(
                    smooth=v, gc_beta=gc_beta,
                    input_map=real_map_vecc,
                    nns_map=nns_map, mm_cond_number=mm_cond_number, nheads=nheads,
                    limit_A=limit_a, limit_B=limit_b, limit_C=limit_c,
                    daily_stride=daily_stride,
                )
                model_cy.precompute_conditioning_sets()

                def nll_cy(p): return model_cy.vecchia_batched_likelihood(p)

                H_cy     = finite_diff_hessian(nll_cy, cy_log_phi)
                H_inv_cy = torch.linalg.inv(H_cy + torch.eye(7, device=DEVICE) * 1e-5)
                torch.cuda.empty_cache()

                beta_hat_cy = model_cy.get_gls_beta(cy_log_phi).detach()

                def per_unit_losses_cy(p):
                    return model_cy.vecchia_per_unit_nll_terms(p, beta_hat_cy)

                _cols_cy = []
                for _k in range(cy_log_phi.shape[0]):
                    _pp = cy_log_phi.detach().clone(); _pp[_k] += _eps
                    _pm = cy_log_phi.detach().clone(); _pm[_k] -= _eps
                    with torch.no_grad():
                        _cols_cy.append(
                            (per_unit_losses_cy(_pp) - per_unit_losses_cy(_pm)) / (2.0 * _eps))
                J_mat_cy = torch.stack(_cols_cy)
                N_units_cy = J_mat_cy.shape[1]
                J_cy     = J_mat_cy @ J_mat_cy.T / N_units_cy ** 2
                torch.cuda.empty_cache()

                GIM_cy = H_inv_cy @ J_cy @ H_inv_cy
                Jac_cy = torch.autograd.functional.jacobian(
                    transform_log_phi_to_physical, cy_log_phi)
                SE_cy  = torch.sqrt(torch.diag(Jac_cy @ GIM_cy @ Jac_cy.T)).detach().cpu().numpy()
                Pt_cy  = transform_log_phi_to_physical(cy_log_phi).detach().cpu().numpy()

                del real_map_vecc, vecc_map, model_cy, H_cy, J_cy, GIM_cy, J_mat_cy
                gc.collect(); torch.cuda.empty_cache()
                ...
                row = {
                    ...
                    'J_method': 'observed', 'gc_beta': gc_beta, 'elapsed': round(elapsed, 1),
                }
                for k, name in enumerate(P_NAMES):
                    ...
                    row[f'CY_Est_{name}'] = round(float(Pt_cy[k]), 6)
                    row[f'CY_SE_{name}']  = round(float(SE_cy[k]), 6)
                records.append(row)

                # Print per-day table
                print(f"\n  {'Param':<10} | {'DW Est':>8} | {'DW SE':>8} || "
                      f"{'VC Est':>8} | {'VC SE':>8} || {'CY Est':>8} | {'CY SE':>8}")
                print(f"  {'-'*80}")
                for k, name in enumerate(P_NAMES):
                    print(f"  {name:<10} | {Pt_dw[k]:>8.4f} | {SE_dw[k]:>8.4f} || "
                          f"{Pt_vc[k]:>8.4f} | {SE_vc[k]:>8.4f} || "
                          f"{Pt_cy[k]:>8.4f} | {SE_cy[k]:>8.4f}")
