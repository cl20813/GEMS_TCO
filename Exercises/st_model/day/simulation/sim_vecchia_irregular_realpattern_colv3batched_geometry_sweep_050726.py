"""
sim_vecchia_irregular_realpattern_colv3batched_geometry_sweep_050726.py

Amarel simulation for real-data-like irregular GEMS observation patterns.

Question:
  With known truth, which batched reverse-L conditioning geometry gives the
  best RMSRE/speed tradeoff relative to Hybrid Lean?
...
Models:
  ...
  2. Column V3 Batched geometry sweep, nominal tail m ~= above + per_lag * 3
"""
...
COLUMN_GEOMETRY_SPECS = [
    {"label": "base_A3_R3_D14", "above_count": 3, "right_col_count": 3, "per_lag_count": 14},
    {"label": "A1_R3_D14", "above_count": 1, "right_col_count": 3, "per_lag_count": 14},
    {"label": "A2_R3_D14", "above_count": 2, "right_col_count": 3, "per_lag_count": 14},
    {"label": "A4_R3_D14", "above_count": 4, "right_col_count": 3, "per_lag_count": 14},
    {"label": "A3_R4_D14", "above_count": 3, "right_col_count": 4, "per_lag_count": 14},
    {"label": "A3_R5_D14", "above_count": 3, "right_col_count": 5, "per_lag_count": 14},
    {"label": "A3_R6_D14", "above_count": 3, "right_col_count": 6, "per_lag_count": 14},
]
...
def fit_hybrid(source_map_ord, nns_grid, initial_vals, args):
    ...
    row = {
        ...
        "above_count": np.nan,
        "right_col_count": np.nan,
        "per_lag_conditioning_count": np.nan,
        "lag_count": np.nan,
...
def fit_column(source_map_ord, ordered_grid_coords_np, initial_vals, args, spec: dict, head_right_cols: int):
    params = [torch.tensor([v], device=DEVICE, dtype=DTYPE, requires_grad=True) for v in initial_vals]
    above_count = int(spec["above_count"])
    right_col_count = int(spec["right_col_count"])
    per_lag_count = int(spec["per_lag_count"])
    lag_count = 2
    total_conditioning = above_count + per_lag_count * (lag_count + 1)
    model_name = (
        f"ColumnV3Batched_Up{above_count}_Right{right_col_count}_"
        f"Down{per_lag_count}_Lag{lag_count}_head{head_right_cols}_realloc"
    )
...
def make_model_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = []
    group_cols = ["model", "kernel", "head_right_cols", "above_count", "right_col_count"]
...
def main() -> None:
    ...
    parser.add_argument("--out-prefix", type=str, default="sim_vecchia_irregular_realpattern_colv3batched_geometry_sweep_050726")
    ...
    for it in range(args.num_iters):
        ...
        fitters = [("hybrid", lambda: fit_hybrid(source_map_ord, nns_grid, initial_vals, args))]
        for spec in COLUMN_GEOMETRY_SPECS:
            fitters.append((
                f"column_{spec['label']}",
                lambda spec=spec: fit_column(
                    source_map_ord,
                    ordered_grid_coords_np,
                    initial_vals,
                    args,
                    spec,
                    args.column_head_right_cols,
                ),
            ))
