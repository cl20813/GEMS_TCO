        ...
        for pat in patience_list:
            for gamm in gamma_list:
                for lrr in lr_list:
                    ...
                    res = alg_optimization( f"2024-07-{day+1}", f"Vecc_ori and b2{0.99}, gamma:{gamm}, pat:{pat}", (200 / lat_lon_resolution[0]) * (100 / lat_lon_resolution[0]) , lrr,  step_size , out, epoch_time, epoch)
                    ...
                    csv_filepath = input_path/f"hyper_parm_opt3_{(200 / lat_lon_resolution[0]) * (100 / lat_lon_resolution[0])}.csv"
