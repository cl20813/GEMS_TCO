"""
sim_dw_grad_2110_lat1_042826.py

3-way Debiased Whittle filter comparison:
  Model 1 — Gradient : vector increments (D_lat X, D_lon X)
  Model 2 — 2-1-1-0  : scalar filter Z=−2X+X↓+X→  (debiased_whittle_2110)
  Model 3 — Lat-1    : scalar filter Z=X↓−X        (debiased_whittle_lat1)

Each model builds its own tapered multivariate sample periodogram and
model-consistent expected periodogram.

Lat ordering: lats_grid ascending (south → north) — consistent with torch.unique.

Usage (local test):
  python sim_dw_grad_2110_lat1_042826.py --num-iters 5 --lat-factor 10 --lon-factor 4

Usage (Amarel):
  python sim_dw_grad_2110_lat1_042826.py --num-iters 300
"""
...
from GEMS_TCO import debiased_whittle_grad_filter as dw_grad
...
MODELS       = ['grad', 'filt_2110', 'lat1']
MODEL_LABELS = ['Gradient', '2-1-1-0', 'Lat-1']
...
def make_plots(df, true_dict, plot_dir, n_iters):
    ...
    tv_list  = [true_dict['sigmasq'],    true_dict['range_lat'], true_dict['range_lon'],
                ...
                true_dict['nugget']]
    COLORS = {'grad': '#7B1FA2', 'filt_2110': '#E53935', 'lat1': '#43A047'}
    iters  = df['iter'].values

    # ── 1. KDE per parameter (4 models overlaid) ─────────────────────────────
    fig, axes = plt.subplots(4, 2, figsize=(16, 20))
    axes = axes.flatten()
    fig.suptitle(f'3-Model DW — Parameter Distributions  ({n_iters} iters)',
    ...
    plt.tight_layout()
    plt.savefig(plot_dir / 'dw_3model_kde.png', dpi=130, bbox_inches='tight')
    plt.close(); print(f"  Saved: dw_3model_kde.png")
    ...
    ax.set_title(f'3-Model DW — RMSRE  ({n_iters} iters)', fontsize=12)
    ax.legend(fontsize=9); ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(plot_dir / 'dw_3model_rmsre.png', dpi=130, bbox_inches='tight')
    plt.close(); print(f"  Saved: dw_3model_rmsre.png")
...
def cli(
    ...
    csv_raw     = f"sim_dw_grad_2110_lat1_{date_tag}.csv"
    csv_summary = f"sim_dw_grad_2110_lat1_summary_{date_tag}.csv"
    ...
    dwl_grad = dw_grad.debiased_whittle_likelihood()
    ...
    for it in range(num_iters):
        ...
        try:
            ...
            # Gradient slices (model 1): component order is
            # D_lat(t0), D_lon(t0), D_lat(t1), D_lon(t1), ...
            db_grad = dw_grad.debiased_whittle_preprocess([reg_agg], [reg_map], day_idx=0,
                                                           **params_kwargs)
            sl_grad = [x.to(DEVICE_DW) for x in db_grad.generate_gradient_filtered_time_slices(
                lat_r[0], lat_r[1], lon_r[0], lon_r[1])]
            if not sl_grad:
                raise RuntimeError("Gradient preprocessing returned no valid time slices.")

            J_grad, n1g, n2g, p_grad, tap_grad, om_grad = dwl_grad.generate_Jvector_tapered_mv(
                sl_grad, dwl_grad.cgn_hamming, LC, NC, VC, DEVICE_DW)
            I_grad  = dwl_grad.calculate_sample_periodogram_vectorized(J_grad)
            ta_grad = dwl_grad.calculate_taper_autocorrelation_multivariate(
                tap_grad, om_grad, n1g, n2g, DEVICE_DW)
            del om_grad
            ...
            J_old, n1o, n2o, p_time, tap_old, om_old = dwl_2110.generate_Jvector_tapered_mv(
                sl_old, dwl_2110.cgn_hamming, LC, NC, VC, DEVICE_DW)
            I_old   = dwl_2110.calculate_sample_periodogram_vectorized(J_old)
            ta_old  = dwl_2110.calculate_taper_autocorrelation_multivariate(
            ...
            J_lat1, n1_l, n2_l, _, tap_lat1, om_lat1 = dwl_lat1.generate_Jvector_tapered_mv(
                sl_lat1, dwl_lat1.cgn_hamming, LC, NC, VC, DEVICE_DW)
            I_lat1  = dwl_lat1.calculate_sample_periodogram_vectorized(J_lat1)
            ta_lat1 = dwl_lat1.calculate_taper_autocorrelation_multivariate(
                tap_lat1, om_lat1, n1_l, n2_l, DEVICE_DW)
            del om_lat1

            print(f"  Gradient:{n1g}×{n2g} p={p_grad}  "
                  f"2110:{n1o}×{n2o} p={p_time}  "
                  f"lat1:{n1_l}×{n2_l} p={p_time}")
            ...
            opt1 = torch.optim.LBFGS(p1, lr=1.0, max_iter=20, max_eval=100,
                                      ...
                                      tolerance_grad=1e-5)
            t0 = time.time()
            _, _, _, loss1, _ = dwl_grad.run_lbfgs_tapered(
                params_list=p1, optimizer=opt1, I_sample=I_grad,
                n1=n1g, n2=n2g, p_time=p_grad, taper_autocorr_grid=ta_grad,
                max_steps=dw_steps, device=DEVICE_DW)
            t1 = time.time() - t0
            rmsre1, est1 = calculate_rmsre([p.item() for p in p1], true_dict)
            print(f"  [grad]      RMSRE={rmsre1:.4f}  ({t1:.1f}s)")
        ...
        # ── Record ───────────────────────────────────────────────────────────
        for m, est, rmsre, loss, t_s, n1_, n2_, p_ in [
            ('grad',      est1, rmsre1, loss1, t1, n1g,   n2g, p_grad),
            ('filt_2110', est2, rmsre2, loss2, t2, n1o,   n2o, p_time),
            ('lat1',      est3, rmsre3, loss3, t3, n1_l,  n2_l, p_time),
        ]:
            ...
            rec[f'n1_{m}'] = n1_; rec[f'n2_{m}'] = n2_; rec[f'p_{m}'] = p_
    ...
    # ── Plots ─────────────────────────────────────────────────────────────────
    try:
        plot_dir = output_path / "plots" / "dw_3model_grad_2110_lat1"
