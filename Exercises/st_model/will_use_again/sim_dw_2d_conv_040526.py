"""
sim_dw_2d_conv_040526.py

Simulation study: Debiased Whittle with 2D separable convolution filter [[-1,1],[1,-1]].

Filter: Z(i,j) = -X(i,j) + X(i,j+1) + X(i+1,j) - X(i+1,j+1)
  |H(w)|^2 = 4sin^2(w1/2) * 4sin^2(w2/2)
  Excludes w1=0 row AND w2=0 column from likelihood.

Output grid after 2D diff:
  (nlat-1) × (nlon-1)

Module: debiased_whittle_2d_conv  (copy of debiased_whittle_lat1d with 2D filter)
  — same API, same generate_Jvector_tapered_mv, same taper_autocorr_grid format
  — only cov_spatial_difference and apply_first_difference_2d_tensor differ

Usage:
  python sim_dw_2d_conv_040526.py --num-iters 5 --lat-factor 10 --lon-factor 4
  python sim_dw_2d_conv_040526.py --num-iters 300
"""
...
DEVICE_DW = torch.device("cpu")   # DW spectral ops always on CPU (avoids CUDA assert)
DTYPE     = torch.float64
...
def cli(
    ...
    print(f"Device     : {DEVICE}  (DW likelihood: {DEVICE_DW})")
    print(f"Filter     : 2D conv [[-1,1],[1,-1]]  Z(i,j)=-X(i,j)+X(i,j+1)+X(i+1,j)-X(i+1,j+1)")
    ...
    csv_raw     = f"sim_dw_2d_conv_{date_tag}.csv"
    csv_summary = f"sim_dw_2d_conv_summary_{date_tag}.csv"

    # ── True parameters (real-data-calibrated, same as filter_comparison) ────
    true_dict = {
        'sigmasq':    13.059,
        'range_lat':  0.154,
        'range_lon':  0.195,
    ...
    for it in range(num_iters):
        ...
        try:
            ...
            # ── DW with 2D conv filter [[-1,1],[1,-1]] (CPU) ─────────────────
            ...
            # generate_spatially_filtered_days applies 2D conv filter:
            ...
            J_vec, n1, n2, p_time, taper, obs_masks = dwl.generate_Jvector_tapered_mv(
                time_slices, dwl.cgn_hamming, LAT_COL, LON_COL, VAL_COL, DEVICE_DW)
            ...
            print(f"  DW grid: {n1}×{n2}, {p_time} time steps  (2D conv)")
